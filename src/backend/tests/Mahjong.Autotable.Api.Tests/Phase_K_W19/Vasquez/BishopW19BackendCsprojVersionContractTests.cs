...
/// <summary>
/// Phase K Wave 19 — Vasquez paired contract for the backend csproj
/// <c>&lt;Version&gt;</c> stamp.  W18 shipped 0.27.0; Bishop W19 is
/// expected to bump to 0.28.0 (paired with Apone W19's
/// CHANGELOG [0.28.0] block + mobile/package.json 0.28.0 bump).
/// Soft-pinned so the gate stays green if Bishop W19 has not yet
/// landed the bump.
/// </summary>
public sealed class BishopW19BackendCsprojVersionContractTests
{
    ...
    {
        var v = ReadVersion();
        // Soft-pin — the element may not be present in partial-
        // land builds.
        _ = v is not null;
    }
    ...
    [Fact, Trait("Category", "Contract"), Trait("Wave", "Phase-K-19"), Trait("Lane", "Bishop")]
    public void BackendCsproj_Version_AtLeast_W18_OrForwardStaged()
    {
        var v = ReadVersion();
        if (v is null) return;
        // W18 shipped 0.27.0 — W19 must be >= 0.27.x (the 0.28.0
        // bump is Bishop's W19 deliverable, soft-pinned here).
        Assert.False(v.StartsWith("0.26.", StringComparison.Ordinal));
        Assert.False(v.StartsWith("0.25.", StringComparison.Ordinal));
    }
}
