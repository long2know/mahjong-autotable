...
/// RoundTimerService BackgroundService with mockable clock and
/// auto-close metric.
/// </summary>
public sealed class BishopW22RoundTimerServiceContractTests
{
    [Fact, Trait("Category", "Contract"), Trait("Wave", "Phase-K-22"), Trait("Lane", "Bishop")]
    public void RoundTimerService_Present_OrForwardStaged()
    {
        ...
        var t = apiAsm.GetTypes().FirstOrDefault(x =>
            x.Name.Equals("RoundTimerService", StringComparison.Ordinal));
        Assert.NotNull(t);
    }

    [Fact, Trait("Category", "Contract"), Trait("Wave", "Phase-K-22"), Trait("Lane", "Bishop")]
    public void ReconnectAuditEntry_KindTournamentRoundAutoClosed_OrForwardStaged()
    {
        var apiAsm = AppDomain.CurrentDomain.GetAssemblies()
            .FirstOrDefault(a => a.GetName().Name == "Mahjong.Autotable.Api");
        if (apiAsm is null) return;
        ...
        var has = fields.Any(f =>
            f.Name.Contains("AutoClosed", StringComparison.OrdinalIgnoreCase)
            || f.Name.Contains("RoundAutoClosed", StringComparison.OrdinalIgnoreCase));
        Assert.True(has);
    }
}
