...
#### 3.4a. W16 re-validation + 26th candidate identification (W16 — Vasquez)

W16 ran the standard 5-run flake harness at the post-Bishop-W16
baseline; **25/25 candidates** from the W12 audit remained migrated
(no W12-set regressions; the Bishop W15 commit's
`[Collection("DbSerial")]` adds on the two `Phase_K_W9/Bishop/`
files held).  Zero flakes observed across the five
`dotnet test src/backend/Mahjong.Autotable.slnx` runs at gate
3621/0/0; the W16 Vasquez gate-snapshot
(`Phase_K_W16/Vasquez/gate-snapshot.txt`) records the harness.

W16 also identified a **new candidate** outside the W12 audit set:

- `src/backend/tests/Mahjong.Autotable.Api.Tests/Phase_K_W16/Bishop/PerTenantRotationAdminControllerTests.cs`
  uses `AppDbContext` + `UseSqlite(…)` in its admin-CRUD harness
  for the W16 PerTenantJwksRotation admin endpoints (the W16
  ReplayRetentionStore EF entity adjunct discussed in the W17
  brief is co-located here via the same `AppDbContext`).

The file is **Bishop-lane** by `wave_subdir_overrides`, so the
attribute application is **Bishop's** to land (mirrors the W15
disposition for the original two W9/Bishop candidates).  W16
recorded the candidate as the **26th** of the running DbSerial
inventory — W12 audit's 25 + this one — without applying the
attribute itself.  Bishop's W17 lane (or later) closes this.

#### 3.4b. W17 re-validation + 27th+ candidate identification (W17 — Vasquez)

W17 re-runs the 5-run flake harness at the post-Bishop-W17
baseline; **all 25 W12 candidates remain migrated** (no
attribute regressions); the W16 26th candidate **also remains
un-migrated** (Bishop W17 did not pick it up; see hand-off in
the next paragraph).  Zero flakes observed across the five
`dotnet test` runs at gate ≥ 3800; the W17 Vasquez gate-snapshot
(`Phase_K_W17/Vasquez/gate-snapshot.txt`) records the harness.

W17 identifies **three new EF-touching test files** added by
Bishop W17 under `Phase_K_W17/Bishop/`:

- `PerTenantRotationDeleteAsyncTests.cs` — uses `AppDbContext`
  via `UseSqlite(…)` to exercise the W17 `DeleteAsync` hard-delete
  contract on `IPerTenantJwksRotationStore`.
- `ReplayRetentionAdminControllerTests.cs` — uses `AppDbContext`
  via `AddDbContext<AppDbContext>(o => o.UseSqlite(…))` to
  exercise the new `/api/admin/replays/retention` CRUD surface
  end-to-end (with audit-write side effects).
- `SignalRRetentionAdminControllerTests.cs` — uses `AppDbContext`
  via `AddDbContext<AppDbContext>(o => o.UseSqlite(…))` to
  exercise the W17 NEW `/api/admin/signalr/retention` CRUD
  surface end-to-end (mirrors the replay-retention controller).

All three are **Bishop-lane** (under `Phase_K_W17/Bishop/`) and
**lack** `[Collection("DbSerial")]`.  The W17 inventory:

| # | File (lane) | Wave introduced | DbSerial applied? |
|---|------------|-----------------|-------------------|
| 1-23 | W12 audit canonical 23 (Vasquez-lane) | W12 audit / W13 apply | YES (W13) |
| 24-25 | `Phase_K_W9/Bishop/{EfCommentaryUsageMeter,IdempotencyStoreContract}Tests.cs` | W12 audit (Bishop-lane); applied W15 | YES (W15) |
| 26 | `Phase_K_W16/Bishop/PerTenantRotationAdminControllerTests.cs` | W16 (new) | NO — Bishop hand-off |
| 27 | `Phase_K_W17/Bishop/PerTenantRotationDeleteAsyncTests.cs` | W17 (new) | NO — Bishop hand-off |
| 28 | `Phase_K_W17/Bishop/ReplayRetentionAdminControllerTests.cs` | W17 (new) | NO — Bishop hand-off |
| 29 | `Phase_K_W17/Bishop/SignalRRetentionAdminControllerTests.cs` | W17 (new) | NO — Bishop hand-off |

**Total candidates as of W17 close: 29** (W12's 25 + W16's 1
+ W17's 3).  Total migrated: 25.  Bishop-lane backlog: 4 files
across W16 + W17.

The W17 brief framed the expected new candidates as "26th
(W16 replay-retention) + 27th (W17 SignalR-retention +
DeleteAsync)".  Empirically the W17 split is finer (three
separate test files, not one), so the inventory grows by **+3
at W17** rather than +1.  All four Bishop-lane open candidates
share the same blocker: a Vasquez-authored
`[Collection("DbSerial")]` edit on a `Phase_K_W17/Bishop/` path
would trip the cross-lane bundling gate (W14 memo §1, W15 §3.4
above).  Application MUST land in a Bishop-authored commit.

**Hand-off to Bishop W18 (or beyond):** apply
`[Collection("DbSerial")]` to all four Bishop-lane open
candidates (the W16 file + the three W17 files).  The
Vasquez-authored W17 contract test
`Phase_K_W17/Vasquez/BishopW16W17DbSerialCandidatesTests.cs`
soft-pins the absence today (records the candidate-file fact);
flips to a hard-asserting positive observation once Bishop
lands the attribute applications.  Same forward-stage pattern
as W15.

**Why the migration backlog is not a regression risk today:**
the W12/W13/W14/W15/W16/W17 5-run flake harnesses have produced
**six consecutive zero-flake runs** across six successive gate
bumps (1880 → 3807 → +1927 tests added).  The migration is a
**defensive-depth** play, not a flake-rescue play — the W17
backlog represents a future-stability liability, not a current
