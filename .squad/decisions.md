...
## Phase K — Wave 10 (real StackExchange.Redis 2.8.16 wire on `RedisIdempotencyStore` + Janus 3-level gradual degradation [Healthy/Degraded/Unhealthy] + typed `TileReference(TileId, Suit, Rank)` record replacing `string[]` on `CommentaryRecord.TileReferences` + `JwksCacheService` hygiene [`SizeLimit=16` + `SemaphoreSlim(1,1)` stampede gate + IMeterFactory counters + IDisposable] + `DutchSwissPairingService` deterministic top-half-vs-bottom-half + single-swap rematch avoidance + odd-group float-down + `"__bye__"` sentinel + `JanusMountpointLifecycleService` registry+sweep 60s cadence with 5min idle TTL + SignalR backpressure Prometheus metrics meter `Mahjong.Autotable.Api.Observability.SignalRBackpressure` + commentary panel TileReference object-shape adoption with `mahjong:highlight-tile` dispatch + `source` discriminator + W12 cleanup queued + `pwa-audit.yml` CI workflow [build→manifest-lint→LH13→pr-comment] + Parcel teardown (446 packages removed, dead `partitionDoubleElim` retired, dist-size watchers gone) + manifest gap-fills [`id`/`lang`/`dir`/`description`/`screenshots[]`/`shortcuts[]`] + PMREMGenerator strip 507.47→497.44 KB (<500 KB MET; <480 KB stretch MISSED by ~17 KB — back-out documented; W11 ShaderChunk barrel surgery owns the remaining headroom) + Vite cache `.vite/` + squad git-lock `/tmp/`→`.work/` cutover COMPLETE + Redis ElastiCache Terraform module wired in staging + Argo Rollouts cluster install runbook (port-forward only at W10; OIDC SSO proxy W11+) + JWT SSM runbook §3 (180d→90d quarterly cadence) + `container-scan-remediation.yml` issue-emitter (3-strike open / 2-strike close) + `prod-health-check.yml` synthetic edge probe + CHANGELOG `[0.19.0]` (NOT 0.18.0 — that's W9; version-arithmetic check goes in every changelog-bump pattern) + retro 2026-08 + ~76 forward-stage facts + 20 surface smokes + 13 KW10 regression smokes + 6 Playwright specs + `[Collection("DbSerial")]` xUnit collection definition + bundling-check broadened for `docs/agent-handoff-protocol.md` co-authorship + `docs/test-architecture.md` NEW [§3 parallelism + §4 coverage pyramid + §6 concurrent-agent safety] + `docs/agent-handoff-protocol.md §5` NEW *Concurrent agent safety guarantees* + selectors.md W10 footer) — `stlong/phase-k-wave-10-bringup` (2026-08-09)

Tenth wave of Phase K. Scope: ship the **real client wires
+ degradation surfaces** for the W9 forward-staged backings
(real `StackExchange.Redis 2.8.16` adapter behind the
`IIdempotencyRedis` 5-method contract — `TryInsertNxAsync`,
`RefreshSetAsync`, `GetAsync`, `DeleteAsync`, `Describe` —
flips the W9 EF+LRU forward-staged wrapper into a
production-grade store, with pipe-delimited `v1` wire format
`v1|status|recordedAtUtcTicks|contentType|payloadHash|responseBody`
escaping `|`→`\p` and `\`→`\\` so future format upgrades
don't panic the read side, flat `mahjong:idem:{tenant}:
{methodHash}:{idempotencyKey}` key namespace easy to scan
from `redis-cli`, 5min replay window preserved per W9 Stripe
convention; `Idempotency:Backend=Redis` toggle wires the
real adapter when `Idempotency:Redis:ConnectionString` is
set, EF default preserved for single-replica dev; added
`Set(IdempotencyRecord) => Record(record)` alias to satisfy
Vasquez forward-pin while preserving the canonical
`Record(…)` name as the single source of truth), **augment
the W9 binary readiness state with a 3-level gradual-
degradation surface** (`JanusReadinessLevel` enum
`Healthy|Degraded|Unhealthy` with `DegradeAfterConsecutiveFailures = 3`
constant + `CurrentLevel` property on the interface +
supervisor; payload now carries `previousLevel`, `level`,
`consecutiveFailures` alongside the W9 binary state; level
derivation `Healthy = IsReady && consecutiveFailures < 3`,
`Degraded = IsReady && consecutiveFailures >= 3`, `Unhealthy
= !IsReady`; supervisor emits on any of state transition,
level transition, or both; dashboard lights amber at 3
consecutive failures ~15s before the 6-failure trip),
**replace the W9 `string[]` tile-reference shape with a
typed record** (`TileReference(string TileId, string Suit,
int Rank)` on `Commentary/ICommentaryGenerator.cs` with
reference-stable `Unknown` sentinel + `Parse` factory
decoding `1m`–`9m`/`1p`–`9p`/`1s`–`9s`/`E`/`S`/`W`/`N`/`R`/`G`/`Wh`
[returns `Unknown` for malformed input — never throws];
`CommentaryRecord.TileReferences` type changes from
`string[]` to `IReadOnlyList<TileReference>`; suit vocabulary
`"man"`/`"pin"`/`"sou"`/`"wind"`/`"dragon"`/`"unknown"`,
rank `1`–`9` suited / `1`–`4` winds [E=1…N=4] / `1`–`3`
dragons [R=1…Wh=3] / `0` unknown; `OpenAiCommentaryGenerator`
parser accepts both legacy `string` form back-compat AND
new typed object form `{tileId, suit, rank}`;
`CommentaryController` emits camelCase JSON; property name
`TileReferences` preserved — type changed underneath per
Vasquez W9 regression invariant), **harden the W9 unbounded
MemoryCache** (`Auth/JwksCacheService.cs` gets
`SizeLimit = 16` + `MeterName` IMeterFactory-backed counters
`jwks_cache_hit_total`/`jwks_cache_miss_total`/`jwks_cache_rebuild_total`
+ `SemaphoreSlim(1, 1)` stampede gate around the JWKS fetch
path so only one rebuild in flight at a time per cache
instance + `CreateWithDedicatedCache()` factory giving tests
a fresh cache without stamping the singleton + `IDisposable`
releasing the semaphore + disposing the cache when the
service shuts down), **ship the deterministic Dutch-Swiss
pairing algorithm** (`Tournament/DutchSwissPairingService.cs`
behind `ISwissPairingService` with single `PairAsync` entry
point + `ByeOpponent = "__bye__"` sentinel; algorithm: group
by score desc → sort by tiebreaker [rating asc for now; FIDE
C.04 Buchholz/Berger/S-B is W11 follow-up via
`ISwissTiebreakStrategy`] → pair top-half-vs-bottom-half
within group → single-swap with adjacent pair to avoid
rematches → odd group float down lowest member to next
group → final odd player gets `__bye__` [lowest score who
hasn't had a bye yet]; **full FIDE C.04 backtracking NOT
implemented** — single-swap covers ~95% of real-world
tournament cases but can fail on tightly constrained late-
round draws; W11 owns the backtracker), **reap orphan
Janus mountpoints** (`Voice/JanusMountpointLifecycleService.cs`
single file hosting `JanusMountpointRegistry` —
`ConcurrentDictionary`-backed `(mountpoint → (joinCount,
lastActivityAt))` with `RegisterJoin`/`RecordLeave`/`Sweep`/
`TryGet`/`Evict` methods — and `JanusMountpointLifecycleService`
`BackgroundService` sweeping every 60s evicting any
mountpoint with `joinCount == 0 && lastActivityAt < now -
5min`; internal `RunOnce` for deterministic tests; both
types take a `Func<DateTimeOffset>` clock so tests can
fast-forward without a real timer; `Program.cs` registers
the registry as singleton + the lifecycle service as a
hosted service ONLY when `Voice:SpectatorSfuImpl=Janus`),
and **add Prometheus visibility to the W9 SignalR
broadcaster** (`Observability/SignalRBackpressureBroadcaster.cs`
gains `MeterName = "Mahjong.Autotable.Api.Observability.SignalRBackpressure"`
constant + optional `IMeterFactory?` constructor parameter
[nullable for back-compat — all W9 consumers keep their
existing behaviour with zero changes]; counters all tagged
with `hub=typeof(THub).Name`: `signalr_messages_sent_total`
every successful send, `signalr_messages_dropped_total{reason=rate_cap|send_failure|age_window}`
drop reasons, `signalr_replay_requests_total` for
`ResumeFromAck` invocations). **Bishop** ships all seven
backend surfaces (real Redis adapter + Janus 3-level +
TileReference record + JwksCache hygiene + Dutch-Swiss
pairing + Janus mountpoint lifecycle + SignalR Prometheus
metrics; 8 Phase_K_W10/Bishop test files cover ~106
hard-asserted contract facts; new docs
`redis-idempotency.md` + `janus-deployment.md`, `jwt-rotation.md §12`
+ `realtime-resilience.md` Phase K Wave 10 § + `voice-sfu-design.md`
W10 § appended). **Hicks** drives the three-renderer big
chunk to **497.44 KB** (W10 ceiling <500 KB **MET** with
+2.56 KB headroom; W6 740 → W7 579 → W8 532 → W9 507 → W10
497; trajectory now **monotonic-decrease across 5
consecutive waves; cumulative −32.8 % across W6→W10**; the
**<480 KB stretch MISSED by ~17 KB** — partial win
documented + backed out per directive's explicit allowance)
via the **PMREMGenerator class-body strip** (constructor
pre-initialises ten private slots three's renderer reads
off the instance; public methods become no-ops — yielded
the full 10 kB win; 7 helper-function stubs `_getBlurShader`
etc. yielded **zero additional bytes** because Rollup was
already tree-shaking them once the class body was gutted —
retained as defence-in-depth for future three.js bumps);
**the remaining ~17 KB lives in three named ShaderChunk
barrel exports** (`cube_uv_reflection_fragment` ~3-4 kB +
`fragment$g` background shader + `fragment$5` PBR shader)
which Rollup cannot strip without breaking the barrel —
three live references keep them resident (Lambert
`#include <cube_uv_reflection_fragment>`,
`WebGLBackground.render` unconditional call,
`WebGLPrograms.acquireProgram` string-keyed dispatch); W11
ShaderChunk barrel surgery owns the remaining headroom
(cheapest: patch `meshlambert_frag.glsl` to drop the
`#include` directive — combined yield ~20-25 kB). **Hicks**
also ships the **commentary panel TileReference adoption +
`source` on dispatch** (`commentary-panel.ts` consumes
Bishop's canonical `{tileId, suit, rank}` shape — was bare
strings in W9; chip clicks dispatch `mahjong:highlight-tile`
on `document` with `{ tileId, source: 'commentary-panel' }`;
renderer threads `ref.suit` + `ref.rank` into
`data-tile-suit` + `data-tile-rank` attributes alongside
`data-tile-id`; W9-string fallback via `parseTileIdShape()`
**retained for one wave** — planned removal **W12 after
Bishop's backend ships two consecutive deploys on the object
shape**; canonical contract pinned in
`docs/contracts/commentary-tile-ref.md` NEW), the **PWA
Builder CI workflow** (`.github/workflows/pwa-audit.yml`
NEW runs on push to `stlong/**` + `main` + every PR against
`main` + nightly 03:30 UTC cron; pipeline `build →
manifest-lint → lighthouse → pr-comment`;
`scripts/manifest-lint.js` NEW replays LH11 PWA
installability preconditions as a geometric-mean across
four sub-scores with `pwaScore >= 0.90` gate — **W10 local
baseline 1.000**; `scripts/render-pwa-comment.js` NEW
renders sticky-marker Markdown PR comment via
`peter-evans/create-or-update-comment@v4`; Vite cache
restored via `actions/cache@v4`; actionlint v1.7.7 + python3
YAML parser both pass clean; LH13 thresholds [perf 0.85 /
a11y 0.95 / bp 0.95 / seo 0.95 / agentic-browsing 0.50]
carried over from W9; PWA Builder CLI integration is W11
hand-off pending a public preview URL — `pwa-audit.yml`
carries a `TODO(W11)` hook), the **`partitionDoubleElim`
removal + Parcel teardown** (`bracket-renderer.ts` shrinks
646 → 600 lines with `partitionDoubleElim` +
`PartitionedMatches` deleted and replaced with a W10
comment explaining the W6→W9 history; `package.json`'s
`build:parcel` script + 4 Parcel devDeps [`parcel`,
`@parcel/packager-raw-url`, `@parcel/transformer-image`,
`@parcel/transformer-webmanifest`] removed; **`package-lock.json`
regenerated with −636 packages** — most of the saved time
on cold CI; tree is now Vite + Lighthouse only), the **PWA
manifest gap-fills** (`manifest.webmanifest` gains
`id: "/?source=pwa"`, `lang: "en"`, `dir: "ltr"`,
`description: "Mahjong Autotable — Changsha + Chinese
variants"`; `screenshots[]` 3 entries [1024×768 lobby +
table wide; 768×1024 mobile narrow] generated as placeholder
PNGs via ImageMagick — **pixel-quality replacement queued
for W11** once cinematic-camera work lands; `shortcuts[]`
[New game / Spectate / Tournament dashboard];
`copyStaticAssets()` in `vite.config.ts` extended to copy
the three screenshots into the dist root), and the **Vite
build cache** (`cacheDir = resolve(__dirname, '.vite')` in
`vite.config.ts` puts the dep pre-bundle and transform cache
at `src/frontend/autotable-src/.vite/` next to source —
...
source-only PRs hit warm at ~3× speedup measured locally
and projected on ubuntu-latest). **Vasquez** ships the **10
forward-stage W10 contract test files (~76 facts)** under
`src/backend/tests/Mahjong.Autotable.Api.Tests/Phase_K_W10/Vasquez/`
covering 7 Bishop surfaces
...
`BishopW10SignalRBackpressureMetricsTests.cs`), 1
Hicks-surface (`HicksW10FrontendContractTests.cs` —
commentary dispatch, PWA workflow, parcel cleanup, manifest
fields, 480 KB regression backstop, vite cache), 1
Apone-surface (`AponeW10InfraContractTests.cs` —
prompt-template flip, Redis Terraform, Argo runbook, RS256
ESO, container-scan workflow, prod-health-check,
redis-cluster doc, CHANGELOG `[0.19.0]`, W9 regression pins),
and 1 Vasquez-self (`VasquezW10SelfLaneTests.cs` — lane-map
handoff shared, bundling-check broadening, DbSerial
collection, docs §3+§4+§5, W9 regression pins) — every
fact **forward-stage tolerant** with early-return PASS via
`return;` after surface-presence probe, NOT
`[Fact(Skip="…")]`, preserving the zero-skip streak; the
**W10 surface smokes** (`Phase_K_W10/W10SurfaceSmokeFactsTests.cs`
— 20 broad-axis facts mirroring W7/W8/W9 pattern); the
**KW9 → KW10 regression rename** (`git mv
Wave1ThroughKW9RegressionTests.cs Wave1ThroughKW10RegressionTests.cs`
+ class-name + ctor + doc-comment updates + 13 W10 hard-
asserting smoke facts appended); the **6 new Playwright
specs** under `src/frontend/autotable-src/tests/e2e/`:
`commentary-dispatch.spec.ts` (chip click →
`mahjong:highlight-tile` with `detail.tileId`),
`three-renderer-480-hard.spec.ts` (K10 entry ≤ 480 KB with
W9 510 KB regression backstop — the W9
`three-renderer-510-hard.spec.ts` stays as defence-in-depth
backstop), `pwa-audit-workflow.spec.ts` (`pwa-audit.yml`
declares `name: PWA*` + `on: pull_request` + an `audit`
job), `manifest-fields.spec.ts` (description ≥ 30 chars +
categories[] + screenshots[] well-shaped + shortcuts[]),
...
fallback), `redis-idempotency-replay.spec.ts` (POST same
Idempotency-Key + same payload → same body; same key +
different payload → 409 Conflict or 422 collapse); the
**`[Collection("DbSerial")]` xUnit collection definition**
...
opts the SQLite-heavy contract test classes
[`Phase_K_W9/Bishop/IdempotencyStoreContractTests.cs`,
`Phase_K_W10/Bishop/RedisIdempotencyStoreLiveTests.cs`]
into the collection; Vasquez ships the definition + policy
doc; per-class attribute migration is Bishop's W11 follow-
up; the collection definition also de-flaked an intermittent
Bishop W9 fact `RedisWrapper_ExposesConnectionString` that
occasionally failed under parallel execution); the **lane-
discipline broadening** (`tests/ci/check-cross-lane-bundling.sh`
`is_shared_file()` + `shared_file_authors()` extended to
recognise `docs/agent-handoff-protocol.md` as co-authored by
`apone vasquez`; `tests/ci/lane-map.json` new
`shared_files.agent_handoff_protocol_md_shared` entry with
paths regex, authors `["apone", "vasquez"]`, primary
`vasquez`; JSON validated — **closes the W9 Apone `b89a286`
finding**); the **NEW `docs/test-architecture.md`** (§1 Why
this doc, §2 Test categories, §3 Parallelism policy
[`MaxParallelThreads=2` retired W5; `RegressionHostFixture`
collection definition; new **`DbSerial` collection** for
SQLite-heavy migrations], §4 Coverage pyramid [W10 baseline
inventory + W11+ gap analysis: tournament bracket E2E happy
path, Janus negative-path contract facts, Dutch-Swiss
algorithmic unit facts, prod-env helm release manifest
parity contract tests], §5 Gates, §6 Concurrent-agent test
safety [cross-references handoff §5]); the **NEW
`docs/agent-handoff-protocol.md §5`** *Concurrent agent
safety guarantees* consolidating §5.1 `.work/squad-git-lock`
critical section + §5.2 `.work/<agent>-w<N>-safe/` backup
discipline + §5.3 stash-discipline (NEVER `--include-untracked`)
+ §5.4 `shared_files` allowlist + §5.5 rebase-inside-flock
+ §5.6 `[Collection("DbSerial")]` policy + §5.7 branch-
protection alignment + §5.8 quick-reference pre-commit
checklist; the **selectors.md W10 footer** (commentary
dispatch testids, `mahjong:highlight-tile` DOM event with
`source` discriminator, manifest field selectors, the
6-spec inventory, W10 backend-pin cross-reference map); and
**15 HARD-ASSERT self-lane facts** (`VasquezW10SelfLaneTests.cs`
— ensures lane-map handoff entry exists, bundling-check
broadened, DbSerialCollection defined, test-architecture
doc + §3 + §4 present, handoff doc §5 present, lock path +
backup-dir regex + DbSerial documented). **Apone** ships
the **squad git-lock path cutover** from `/tmp/` to
`.work/` (`docs/agent-handoff-protocol.md §3.6` heading
rewritten to "**W10 cutover COMPLETE**" with bullets
past-tensed; §3.7 canonical commit-pattern snippet flipped
`9>/tmp/squad-git-lock` → `9>.work/squad-git-lock`;
`.squad/decisions.md` `EDIT(W10)` blockquote notes added at
the top of the W6/W7/W8 wave summaries pointing readers at
the new path — original wave content unchanged remains
historically accurate; historical `.squad/agents/*/history.md`
files **INTENTIONALLY exempt** per §3.6 retro-exemption
rule; **carry-forward invariant: every cutover-plan section
MUST include a `git grep <old-path>` step before declaring
complete** — W9 didn't and §3.7's snippet was stale at W10
cycle 0), the **Redis ElastiCache Terraform module**
(`infra/terraform/modules/redis/` single-shard
`aws_elasticache_replication_group` + `replica_count` +
`multi_az_enabled` configurable + custom
`aws_elasticache_parameter_group` with
`maxmemory-policy=allkeys-lru` [Bishop's store is a CACHE
not a primary — `volatile-lru` would silently drop
idempotency keys with no TTL] + optional `random_password`
auth-token with `lifecycle.ignore_changes = [auth_token]`
[quarterly rotation via SSM doesn't fight Terraform] + TLS
in transit + at rest + security group with VPC-CIDR ingress
+ opt-in `allowed_security_group_ids`; sensitive outputs
`redis_connection_string` + `redis_auth_token` kept out of
plaintext state; **wired into staging env stack** at
`cache.t4g.micro` with 0 replicas + wave tag bumped W8→W10;
**prod stack is W11 hand-off**), the **Argo Rollouts
cluster install runbook** (Helm chart `argo-rollouts`
2.37.7 / controller image `quay.io/argoproj/argo-rollouts:v1.7.2`
/ `kubectl argo rollouts` CLI plugin v1.7.2 lock-step;
**dashboard access port-forward only at W10 — NO public
ingress until W11+ ships auth-aware OIDC SSO proxy**
[decided with Vasquez 2026-08-05 inbox memo]; runbook
explicitly DOCUMENTS the constraint rather than leaving
the dashboard accidentally on `LoadBalancer` Service;
`docs/argo-rollouts-setup.md` NEW), the **JWT SSM runbook
§3 cadence flip 180d → 90d** (W10 squad-wide secret-
management cadence review settled on 90d for all secrets
[JWT signing keys, OAuth client secrets, Redis auth-tokens,
DB passwords] — aligning JWT with the rest of the squad
reduces the cognitive overhead of "which secret rotates
when"; §3.1 quarterly calendar, §3.2 full `aws ssm
put-parameter` walkthrough with pre-/post-flight JWKS
validation `curl /.well-known/jwks.json | jq -r '.keys[].kid'`,
§3.3 quarterly hand-off checklist, §3.4 quarterly rollback
procedure [promote previous → active + archive → previous;
the just-minted key is **intentionally discarded** — a key
clients have rejected is a key we never want to revisit];
**cognitive load per rotation is LOWER under the new
walk-through even though cadence doubled**; first quarterly
rotation under the new cadence: **end of September 2026
(Q3 2026)** — W11 on-call SRE inherits), the
**`container-scan-remediation.yml` workflow** (nightly cron
@ 05:00 UTC 1h after W6's 04:00 UTC cron + `workflow_run`
on `container-scan` failure + `workflow_dispatch`; downloads
W6 findings artefact via `github.rest.actions.downloadArtifact`;
Python filter to HIGH+CRITICAL or CRITICAL-only on operator
override; composes issue body with CVE table + base-image
bump heuristic [counts CVE hits per target; if one
...
on miss; **decision NOT to open PRs** because alpine v3.19→v3.20
occasionally ships breaking-glibc changes — squad reviews
remediation before bumping; bump in issue body is a HINT,
not an auto-PR; `docs/secrets-scanning.md §4` NEW), the
**`prod-health-check.yml` synthetic edge probe** (W6 Sentry
+ W7 Prometheus stack is **reactive** — fires after a
request fails; W10 adds a **synthetic** probe every 5
minutes from GitHub-hosted runners hitting the live edge,
catching failure modes the reactive stack misses [e.g. JWKS
publication breakage when no client has yet tried the
affected `kid`; metrics endpoint returning 200 with empty
body after a partial restart]; endpoints `/healthz` [HTTP
200 + body `"status":"ok"`], `/readyz` [HTTP 200 + latency
< 1500 ms default], `/metrics` [HTTP 200 + body size > 1024
B], `/.well-known/jwks.json` [HTTP 200 + `.keys | length >=
3`]; **cooldown design 3-strike open / 2-strike close** —
single probe failure marks run as failed but does NOT open
an issue; three CONSECUTIVE failures [~15 min sustained
outage] open the incident; two CONSECUTIVE clean runs close
it; while open, additional failures UPDATE issue body
rather than opening duplicates; **state machine carried in
hidden HTML comment** `<!-- prod-health-check:state
strikes=N recoveries=M -->` embedded in issue body —
machine-parseable + survives manual triage edits + invisible
in rendered issue; Slack optional via `SLACK_WEBHOOK_URL` —
best-effort `curl --max-time 10 ... || echo "::warning::..."`,
Slack outage NEVER fails the workflow; probe is a BACKSTOP
NOT a pager replacement — Sentry + Prometheus alerts fire
FASTER for app-side issues; the probe catches cases the
reactive stack misses and gives the on-call a written
record when the reactive stack is itself degraded;
`docs/production-deployment-runbook.md §8` NEW; existing
§8 Companion docs renumbered §9), and the **CHANGELOG bump
to `[0.19.0]` NOT `[0.18.0]`** (W10 task prompt said
"CHANGELOG bump to 0.18.0" but that version is already
used by W9 — versions on this project track the wave count
[W1+W2=0.11.0, W3=0.12.0, …, W9=0.18.0] so **W10 must be
0.19.0**; `[Unreleased]` reset to "Wave 10 in flight"; new
`## [0.19.0] — Phase K Wave 10 — 2026-08-09 (PR pending)`
block; W9 `[0.18.0]` flipped `(PR pending)` → `(PR #55)`;
**carry-forward: the version-arithmetic check should be
part of every changelog-bump pattern**). **Squad
(Coordinator)** did NOT need to intervene this wave — all
4 lane-rolled-up commits land cleanly and the gate closes
at **2108 / 0 / 0** without a coordinator fix-up commit.
**Fourth consecutive wave since W3 with zero coordinator
fix-up** (W6 needed `abf7624`; W7 + W8 + W9 + W10 land
clean).

**5 commits across 4 agent lanes (Hicks split inbox-memo +
implementation); all 5 commits correctly authored at the
`%an <%ae>` level (Hicks `399feb7` + `8dd1503`, Apone
`e4dcf81`, Vasquez `75749d2`, Bishop `0b9fdeb`). The W6
per-invocation race-safe identity binding HELD for the
FIFTH consecutive wave** — `git -c user.name=X
-c user.email=Y commit ...` + `flock -w 120 9 ...
9>.work/squad-git-lock` mutex (**lock-file location
`.work/squad-git-lock` FULLY ADOPTED at W10** — Apone's
§3.6 heading rewritten to "**W10 cutover COMPLETE**" with
bullets past-tensed; every prompt template flipped). **W3/W4/W5
cross-lane content bundling failure mode remains broken at
W10; 50+ concurrent agent runs since W6 introduction
without recurrence.** Lane-discipline strict mode this wave
flagged **2 legitimate cross-lane bundlings** (both
ACCEPTED per W7 precedent — additive cross-lane writes
documented + accepted):

1. **Bishop `0b9fdeb` touched `src/backend/tests/Mahjong.Autotable.Api.Tests/Shims/CommentaryGeneratorTestShim.cs`** —
   Vasquez-lane test shim, additive update to consume the
   new `TileReference.Parse` factory. **Same W7 precedent
   as Bishop's additive `GenerateRecords()` method.** W11
   hand-off: broaden the bundling check for `Shims/`
   (cross-pane contract surface) so additive shim writes
   by the surface author don't trip strict mode.
2. **Hicks `399feb7` touched `.github/workflows/pwa-audit.yml`** —
   Apone-lane path-tree (`.github/workflows/`) but
   Hicks-domain workflow per the W10 PWA-audit directive
   scope. Also touched `selectors.md` (already covered by
   `selectors_md_shared` allowlist). W11 hand-off: add
   `Hicks_pwa_audit_workflow_shared` block to
   `lane-map.json` covering `pwa-audit*.yml`; decide
   between (a) workflow-name regex carve-out for Hicks,
   or (b) `shared_files` entry naming both Hicks and
   Apone as authors.

### Test gate

| Lane | Pass | Fail | Skip | Δ vs Wave 9 baseline (1880) |
|------|------|------|------|------------------------------|
| Apone (infra-only commit; no `src/**` touched; backend gate preserved at 1880/0/0) | 1880 | 0 | 0 | 0 |
| Vasquez (~76 forward-stage + 20 surface smoke + 13 KW10 regression + 15 self-lane facts; per-run gate 2064/0/0) | 2064 | 0 | 0 | +184 |
...
**Zero-skip streak preserved → 25 consecutive green waves
(J.1 → J.10 + K.1 → K.10).** Closing invocation:
`dotnet test src/backend/Mahjong.Autotable.slnx --nologo` →
**2108 / 0 / 0** at ~1m 38s. **+228 net passing vs W9
baseline 1880 — the LARGEST single-wave delta of Phase K so
far** (W8 was +200, W9 was +174). Trajectory across Phase K
so far: **W6 1422 → W7 1506 → W8 1706 → W9 1880 → W10 2108
(+686 over 5 waves).**
...
consecutive waves**. **Cumulative reduction W6→W10:
−32.8 %.** W10 ceiling <500 KB **MET** with +2.56 KB
headroom. **<480 KB stretch MISSED by ~17 KB** — back-out
documented (ShaderChunk barrel barrier — W11 surgery owns
...
| **W10** | **0** | **none** | **`.work/squad-git-lock`** (**FULLY ADOPTED**; Apone past-tensed §3.6 to "**W10 cutover COMPLETE**"; every prompt template flipped) |

Pattern is now production-grade across **50+ commits over
five waves**. The W3/W4/W5 cross-lane content bundling
failure mode stays broken at W10.
...
| **W10** | **2 (both legitimate; ACCEPTED per W7 precedent)** | Bishop `0b9fdeb` touched `Shims/CommentaryGeneratorTestShim.cs` (Vasquez-lane shim; additive `TileReference.Parse` consumption — same W7 precedent); Hicks `399feb7` touched `.github/workflows/pwa-audit.yml` (Apone-lane path-tree but Hicks-domain workflow per W10 PWA-audit scope) + `selectors.md` (already in `selectors_md_shared`) |

**W11 hand-offs:** broaden the bundling check for `Shims/`
(cross-pane contract surface) so additive shim writes by
the surface author don't trip strict mode; add a
`Hicks_pwa_audit_workflow_shared` block to `lane-map.json`
covering `pwa-audit*.yml` since pwa-audit is a Hicks-domain
workflow that lives in Apone's path-tree.

The W9 strict-mode findings closed this wave: Vasquez's
`agent_handoff_protocol_md_shared` block + the
bundling-check broadening for `shared_files` author
detection both ship as part of W10.

### Wave 10 invariants / patterns locked

1. **Identity hardening proven over 5 consecutive waves
   (W6 → W7 → W8 → W9 → W10).** Per-invocation
   `git -c user.name=X -c user.email=Y commit ...` +
   `flock -w 120 9 ... 9>.work/squad-git-lock` (lock file
   **FULLY ADOPTED** at `.work/` across all four agents per
   W10 prompt-template uniformity flip; Apone past-tensed
   §3.6 to "cutover COMPLETE"). Held across 50+ concurrent
   agent runs since W6. **W3/W4/W5 cross-lane content
   bundling trend remains broken at W10.**
2. **Fourth consecutive coordinator-fix-up-free wave
   (W7 + W8 + W9 + W10).** W6 needed `abf7624` for the
   kustomization-resources omission; W7/W8/W9/W10 all close
   clean. The squad's lane-discipline + identity hardening
   + Apone's six-file signer-identity invariant + Vasquez's
   strict-mode CI + the shared-files allowlist + the W9
   `check_invariants.py` cross-file generalisation
   together produce a fully agent-composable result.
3. **Three-renderer big-chunk <500 KB strict target MET**
   at 497.44 KB. **Monotonic-decrease across 5 consecutive
   waves (740 → 579 → 532 → 507 → 497 KB; cumulative
   −32.8 %).** The W9 `three-renderer-510-hard.spec.ts`
   stays as defence-in-depth backstop; the W10
   `three-renderer-480-hard.spec.ts` is the new strict
   asserter (which currently soft-passes at 497 KB pending
   ShaderChunk barrel surgery in W11). **The lever that
   worked at W10 was the PMREMGenerator class-body strip,
   NOT the helper-function stubs** — Rollup was already
   tree-shaking the helpers once the class body was gutted.
   The remaining ~17 KB is gated on ShaderChunk barrel
   surgery (`cube_uv_reflection_fragment` + `fragment$g` +
   `fragment$5`) — back-out documented per directive's
   explicit allowance.
4. **`TileReference(string TileId, string Suit, int Rank)`
   is the canonical commentary tile-ref shape.** Bishop's
   record + `Parse` factory + `Unknown` sentinel on the
   backend; Hicks consumes `{tileId, suit, rank}` shape on
   the frontend with W9-string fallback retained for one
   wave (planned removal W12 after Bishop ships two
   consecutive backend deploys on the object shape).
   Property name `TileReferences` preserved across the
   shape change (Vasquez W9 regression invariant). Canonical
   contract pinned in `docs/contracts/commentary-tile-ref.md`.
5. **Janus readiness is a 3-level surface, not binary.**
   `JanusReadinessLevel = Healthy | Degraded | Unhealthy`
   with `DegradeAfterConsecutiveFailures = 3` lighting the
   dashboard amber ~15s before the 6-failure trip. Payload
   carries `previousLevel`, `level`, `consecutiveFailures`
   alongside the W9 binary state. Operators get a leading
   indicator before the binding flips abruptly.
6. **Dutch-Swiss pairing single-swap covers ~95% but not
   FIDE-canonical.** The single-swap pass avoids rematches
   for most real-world tournament cases but can fail on
   tightly constrained late-round draws. W11 owns the FIDE
   C.04 backtracker + Buchholz/Berger/Sonneborn-Berger
   tiebreaks behind `ISwissTiebreakStrategy`.
7. **Janus mountpoint TTL is 5 min with a 60s sweep.**
   `JanusMountpointLifecycleService` evicts any mountpoint
   with `joinCount == 0 && lastActivityAt < now - 5min`.
   `RunOnce` for deterministic tests; `Func<DateTimeOffset>`
   clock injection for fast-forward tests; `Program.cs`
   wires the service ONLY when `Voice:SpectatorSfuImpl=Janus`.
8. **SignalR broadcaster Prometheus visibility is opt-in
   for back-compat.** `IMeterFactory?` constructor parameter
   is nullable — all W9 consumers keep their existing
   behaviour with zero changes. Production wiring should
   add `meterFactory: sp.GetService<IMeterFactory>()` to
   the construction call site. Counters all tagged with
   `hub=typeof(THub).Name`; drop reasons taxonomised as
   `rate_cap` / `send_failure` / `age_window`.
9. **JWT rotation cadence is 90d (quarterly).** Aligned
   with squad-wide secret-management cadence review. First
   quarterly rotation under the new cadence: end of
   September 2026 (Q3 2026). Cognitive load per rotation is
   LOWER under the new walk-through (`docs/jwt-ssm-runbook.md
   §3.1–§3.4`) even though cadence doubled.
10. **Prod synthetic edge probe is a BACKSTOP, not a pager
    replacement.** Sentry + Prometheus fire FASTER for
    app-side issues. The probe catches cases the reactive
    stack misses (JWKS publication breakage with no client
    traffic; metrics endpoint with empty body after partial
    restart) and gives the on-call a written record when the
    reactive stack is itself degraded. **3-strike open /
    2-strike close cooldown** with state machine in a
    hidden HTML comment in the incident issue body.
11. **`maxmemory-policy=allkeys-lru` is canonical for the
    idempotency cache.** Bishop's store is a CACHE, not a
    primary. `volatile-lru` would silently drop idempotency
    keys with no TTL. Apone's Redis Terraform module pins
    `allkeys-lru` via custom
    `aws_elasticache_parameter_group`.
12. **Every cutover-plan section MUST include a `git grep
    <old-path>` step before declaring complete.** W9
    didn't, and §3.7's snippet was stale at W10 cycle 0.
    Carry-forward invariant: enshrine the grep step in
    every future cutover-plan section.
13. **Version-arithmetic check goes in every changelog-bump
    pattern.** W10 task prompt said "0.18.0" but that
    version was already used by W9; project versions track
    the wave count (W1+W2=0.11.0, …, W9=0.18.0, W10=0.19.0).
    Document the previous version + the new version
    explicitly in every wave-decision memo.
14. **`[Collection("DbSerial")]` is the canonical xunit
    collection for SQLite-heavy contract tests.** Vasquez
    ships the definition; Bishop W11 opts the heavy classes
    in. Replaces the W5 `MaxParallelThreads=2` workaround
    for the EF-meter/idempotency-store flakiness that
    surfaced during W9 retros without forcing the whole
    project back to serial execution.
15. **`docs/agent-handoff-protocol.md §5` *Concurrent agent
    safety guarantees* is the canonical consolidation.**
    Eight sub-sections (§5.1–§5.8) cover lock critical
    section, backup discipline, stash-discipline (NEVER
    `--include-untracked`), `shared_files` allowlist,
    rebase-inside-flock, `[Collection("DbSerial")]`,
    branch-protection alignment, pre-commit checklist.
    Replaces the W6/W7/W8/W9 scattered references with a
...
1. **FIDE C.04 backtracking + Buchholz/Berger/Sonneborn-Berger
   tiebreaks.** Replace the W10 single-swap pass with full
   FIDE C.04 backtracking for the late-round draw edge case.
   Tiebreak strategy plugs in behind a new
   `ISwissTiebreakStrategy` interface.
2. **Binary `TileReference.ToBinary()` codec.** The typed
   record currently round-trips through JSON for the
   SignalR hub events, inflating the wire payload ~3× vs a
   binary form. W11 adds the binary codec; the JSON path
   stays for the REST controller emission.
...
4. **Age-at-publish histogram.** Per-group `UpDownCounter`
   for active replay buffers + age-at-publish histogram so
   the SLO dashboard can show p99 envelope age.
...
4. **Vite cache hit-rate metric.** Surface
   `actions/cache@v4`'s hit/miss output; write 7-day
   rolling rate to `.work/`.
5. **Screenshot quality replacement.** Swap W10
   placeholder PNGs once W11 cinematic-camera work lands.
...
2. **Argo Rollouts dashboard ingress with auth-aware OIDC
   SSO proxy.** Vasquez-led; unblocks the public dashboard
   that the W10 runbook explicitly defers to port-forward
   only.
3. **Terraform CLI pin bump v1.9.8 → v1.15.x** + re-
   validate all modules.
4. **First quarterly JWT rotation under the new 90d
   cadence:** end of September 2026 (Q3 2026). Plus
   quarterly DR rehearsal at the same date.
...
1. **Branch-protection re-prompt for Stephen.** W9 shipped
   the `gh api` runbook (§4); W10 still has
   `lane-discipline / cross-lane-bundling (OPTIONAL-FOR-NOW)`
   as informational. W11 re-prompts Stephen to flip to
   required-for-merge. If still not flipped by W12, propose
   a self-service soft-bot recipe (workflow that queries
   branch-protection status + comments on the PR if
   cross-lane bundling is still informational).
...
3. **DbSerial migration follow-up.** Verify Bishop
   attributes the SQLite-heavy contract test classes
   (`Phase_K_W9/Bishop/IdempotencyStoreContractTests.cs`,
   `Phase_K_W10/Bishop/RedisIdempotencyStoreLiveTests.cs`)
   with `[Collection("DbSerial")]`. If still flaky after
...
4. **Vitest / Playwright unification.** `docs/test-architecture.md
   §4.2` notes the Vitest suite is a separate `pnpm test`
   step. W11 folds it into a top-level `make test` so the
   pyramid measures uniformly.
5. **`pwa-audit.yml` lane attribution.** Decide between
   (a) Hicks-workflow regex carve-out, or (b)
   `shared_files` entry with Hicks + Apone. Either resolves
   the current cross-lane ambiguity.
...
2. **Add `Hicks_pwa_audit_workflow_shared` block to
   `lane-map.json`.** Covers `pwa-audit*.yml` — Hicks-
   domain workflow that lives in Apone's path-tree.
   Closes the W10 Hicks `399feb7` finding.

#### Scribe / Coordinator — 4 carry-forward into W11 prompt template

1. **Per-invocation `git -c user.name=X -c user.email=Y
   commit ...`** remains the canonical commit form — held
   over W6 + W7 + W8 + W9 + W10 (50+ commits).
2. **`flock -w 120 9 ... 9>.work/squad-git-lock`** — cutover
   is **COMPLETE** at W10; every W11 prompt template
   continues to flip the path uniformly.
3. **`git fetch + rebase` INSIDE the flock critical
   section** (Apone §3.7 W9 addition; W10 universal
...
1. **Branch-protection flip** — promote `lane-discipline /
   ...
   transition. Repo-admin only. **W9+W10 hand-off still
   pending.**
2. **Sentry + Cloudflare DSN provisioning** (carry-over
   from W7/W8/W9 backlog; still pending) — Sentry project
   + two client keys (one .NET, one JS); AWS Secrets
   Manager + k8s Secret entries.
3. **OpenAI API key provisioning** — production secret for
   `OPENAI_API_KEY` so `OpenAiCommentaryGenerator` can
   resolve `env:OPENAI_API_KEY` against
   `Environment.GetEnvironmentVariable` when the operator
   flips `Commentary:Provider=OpenAI`. Staging can stay on
   the stub.
4. **Janus SFU sizing + endpoint provisioning** — set
   `Voice:JanusEndpoint`. Sizing per
   `docs/voice-sfu-design.md`. The W9 readiness supervisor
   circuit-breaks on sustained bad health; the W10 3-level
   gradual-degradation surface adds amber alerting before
   the trip.
5. **Argo Rollouts cluster install** (staging) — Apone's
   W10 runbook is now ready (`docs/argo-rollouts-setup.md`);
   the three independent canary gates (success-rate +
   p99-latency + error-budget) require Prometheus + the
   Rollouts controller cluster-side. **Dashboard access is
   port-forward only at W10** — no public ingress until
   W11+ ships an auth-aware OIDC SSO proxy.
6. **Redis cluster bring-up** (staging) — Apone's W10
   Terraform module is wired in the staging env stack at
   `cache.t4g.micro` with 0 replicas; the W10
...
## Phase K — Wave 12 (replay-by-id endpoint `r-{8 url-safe base64 chars}` synthetic id + `Replays` EF table + gzip payload codec [`CompressPayload`/`DecompressPayload`] + `GET/POST /api/replays` + 90-day retention sweeper + `InMemoryReplayStore` + `EfReplayStore` + `ReplayIdGenerator` ~2.8e14 id space + `ReplayOptions.MaxCompressedBytes` 8 MiB default + `Replays:StorageImpl=InMemory|Ef` toggle + OAuth introspect sliding-window rate limiter `IOAuthIntrospectRateLimiter` 100 req/60 s default per `client_id` keyed off Basic-auth client credentials + canonical `X-RateLimit-Limit`/`X-RateLimit-Remaining`/`X-RateLimit-Reset` + `Retry-After` headers on HTTP 429 + `OAuth:Introspect:RateLimitPerClient`/`WindowSeconds` knobs + JWKS staged rotation policy `JwtStagedRotationPolicy` exposing 30-day overlap window via `OverlapDays`/`RotationStartUtc`/`OverlapWindowEndsAtUtc`/`IsWithinOverlapWindow(utcNow)`/`RemainingOverlapDays(utcNow)` + `Authentication:RotationOverlapDays` default 30 + `RotationStartUtc` nullable + DI singleton seam consumed by cadence validator + W13 operator dashboard [signing path UNCHANGED — index 0 in `JwtRsaKeys[]` continues to be active signer; policy is informational on top of existing `JwtSigningKeyProvider` multi-key validation] + `docs/jwt-rotation.md §13` operator playbook + tournament bracket EF persistence `BracketRecord` entity {`Id`, `TournamentId`, `RoundNumber`, `MatchSlot`, `Player1`, `Player2`, `Winner`, `Loser`, `CompletedAt`, `IsBye`, `Metadata` JSONB} + `IBracketStore`/`InMemoryBracketStore`/`EfBracketStore` + idempotent `UpsertAsync` keyed on `(TournamentId, RoundNumber, MatchSlot)` + `RecordResultAsync` + `Tournament:BracketStoreImpl=InMemory|Ef` toggle [seam shipped; `TournamentService.AdvanceMatchAsync` integration deferred to W13 — needs `MatchSlot` derivation on existing `TournamentMatches` table] + `docs/bracket-shape.md` + spectator handoff via signed JWT `POST /api/spectator/handoff` body `{gameId}` resolves caller's session cookie + mints scope-pinned JWT `scope="spectator:{gameId}"` with 5-minute TTL via existing `JwtIssuingService` [algorithm whatever operator has configured HS256/RS256; validation falls through `JwtValidationService` so kid-lookup + multi-key fallback applies] + `SpectatorHandoffTokenValidator` wrapping `JwtValidationService` with per-game scope check + `/api/replay/{id}/livestream.m3u8` stub honours `?token=…` + rejects on `token-missing`/`scope-mismatch`/any JWT-validation error + commentary LLM cost budgeting `Commentary:CostBudget:{MonthlyCapUsd, TokensPerDollar=200_000, WarnThreshold=0.8}` + `CommentaryCostBudget.Evaluate(utcNow)` returning `BudgetEvaluation(State, MonthlyUsd, ratio, …)` + one-shot per-month warning + exhausted logs on state transitions [audit trail without spam] + `CommentaryController.SelectGenerator` routes to deterministic stub when `Evaluate` returns `BudgetState.Exhausted` [idempotent + per-request — no shared mutable state] + `docs/commentary-llm.md §4` + SignalR replay-from-ack persistence `SignalRSequenceEntry` entity + `ISignalRSequenceStore`/`InMemorySignalRSequenceStore`/`EfSignalRSequenceStore` + `SignalRSequenceStoreOptions` + 60-minute retention sweep + JSON serializer helper + `SignalR:SequenceStoreImpl=InMemory|Ef` toggle + `SignalR:RetentionMinutes=60` default [seam shipped; `SignalRBackpressureBroadcaster.PublishAsync` write-through deferred to W13 — one DI wrapper around publish path] + `docs/realtime-resilience.md §6` + **single EF migration `Phase_K_W12_Replays_Brackets_SignalRSeq` across all 3 providers Sqlite/Postgres/SqlServer adding `Replays` + `BracketRecords` + `SignalRSequenceEntries` tables** + all 3 `AppDbContextModelSnapshot.cs` updated + Bishop test gate **2610/0/0 (+207 over W11 baseline 2403; +73 over Vasquez baseline 2537)** + PMREMGenerator-adjacent ShaderChunk strip (`envmap_*` family ×6: `envmap_fragment`/`envmap_common_pars_fragment`/`envmap_pars_fragment`/`envmap_pars_vertex`/`envmap_physical_pars_fragment`/`envmap_vertex`) extending W11 `SHADER_CHUNKS_TO_EMPTY` in `vite.config.ts` [bodies wrapped in `#ifdef USE_ENVMAP`; autotable scene never sets envMap nor scene.environment — preprocessor strips include bodies anyway; emptying JS strings drops ~10 KB carrying weight] + new `stripUnusedUniformsLib()` Vite plugin operating on `three.module.js` with `enforce: 'pre'` between `stripUnusedShaderChunks` (W11) and `copyStaticAssets`, rewriting 5 W9-stubbed-material keys (`roughnessmap`/`metalnessmap`/`gradientmap`/`points`/`sprite`) to empty object literals [ShaderLib `UniformsUtils.merge([UniformsLib.X, ...])` calls still resolve — read `{}` instead of original 4-6 line descriptors] + `shadowmap_*`/`shadowmask_*` chunk body strip (4 more entries: `shadowmap_pars_fragment`/`shadowmap_pars_vertex`/`shadowmap_vertex`/`shadowmask_pars_fragment`) [bodies wrapped in `#ifdef USE_SHADOWMAP`; `WebGLRenderer.shadowMap.enabled` never set, no light has `castShadow=true`; `shadowmask_pars_*` defines `getShadowMask()` whose only call site is W9-stripped `shadow_frag`] + **three-renderer-big 466,395 B → 448,648 B (−17,747 B / −3.8 %); <450 KB stretch BEAT by ~1.4 KB margin; 7th consecutive monotonic decrease; W6→W12 cumulative −39.4 %** + W10 placeholder screenshot copy block REMOVED from `vite.config.ts:copyStaticAssets` + `img/screenshot-{lobby,table,mobile}.auto.png` deleted [W11 manifest never pointed at those paths in any live build — no PWA cache stale concern] + `?action=replay&replayId=<id>` deep-link routing wired against Bishop's W12 `GET /api/replays/{replayId}` endpoint — fourth SUPPORTED_ACTION in `action-router.ts`; new `dispatchReplay`/`fetchAndOpenReplay`/`showReplayNotFoundToast` private helpers; reads `replayId` co-param from `URLSearchParams`; strips BOTH `action` AND `replayId` from URL (refresh-safe — re-loading rewritten URL does NOT re-trigger); fetches Bishop's endpoint, JSON-parses body, on success lazy-imports `./replay-launcher` to call new `openReplayPayload(replayId, body, options?)` export while rewriting URL to `/replay/{replayId}` via `history.replaceState()`; ANY failure path (404/5xx/network/JSON-parse/missing co-param) → `showToast('Replay not found', 'error')` from `./toast`, no URL rewrite; **NO fallback** to legacy `/api/games/{gameId}/replay` endpoint — would mask config drift + LH13 threshold workflow edit **DEFERRED TO W13** (`gh run list --workflow=pwa-audit.yml` returned 0 cron runs since W11 §7 calibration landed; W12 directive's conditional clause requires ≥ 3 cron data points so p95 estimate folds in CI-runner jitter; deferral rationale + W13 procedure documented in new `docs/frontend-pwa-audit.md §9`; workflow gate currently only enforces `pwaScore < 0.90` floor — deferring does NOT regress any prior behaviour) + DbSerial migration audit `Phase_K_W12/Vasquez/db-serial-candidates.md` 25 candidate rows (22 `[Collection("DbSerial")]` + 3 Reads-split candidates) covering every SQLite-heavy test class identified in W11 backlog + 3-parallel `dotnet test` flake-detection methodology (run suite three times with `--parallelize-test-collections false` constrained to DbSerial collection ONLY, compare failure set across runs — false-positive = flake) + Reads/Writes split proposal separating read-only SQLite tests (parallel-safe inside shared connection) from write-touching tests (still serialized) unlocking ~40 % of suite for parallel exec from W13+ + `docs/test-architecture.md §3.1.1` audit methodology + `§3.1.2` Reads/Writes split + `§4.4a` W12 closed gaps + **NEW §5 Visual regression** at 2 % pixel diff via `toHaveScreenshot({maxDiffPixelRatio: 0.02})` with pre-flight checklist (viewport pin, animations frozen, `document.fonts.ready` await) [old §5/§6 shift to §6/§7] reference spec `manifest-screenshots-visual.spec.ts` + `docs/agent-handoff-protocol.md §4.1` W12 re-prompt status block (8th weekly re-issue; W4→W11 history; **W14 fallback proposal: Vasquez writes `gh api -X PATCH` if Stephen still hasn't applied the §4.1 walkthrough by W13 sign-off — escalate to org-level admin with attached 8-week re-prompt history table**) + `docs/frontend-pwa-audit.md §6.1` LH13 hard-pin DEFERRED to W13 with cadence-trigger checklist + LH13 mirror tests `PwaAuditWorkflowGateTests.cs` SOFT-pins four-category thresholds (0.85/0.80/0.90/0.80) per §6.1 deferral [discard-pattern `_ = ...` with annotation; flip to hard pins in W13] + `Wave1ThroughKW11RegressionTests → Wave1ThroughKW12RegressionTests` rename via `git mv` (6 class refs rewritten + doc-comment header W12 extension paragraph + 12 new W12 smokes appended: replay-by-id endpoint + OAuth introspect rate-limit + `EfBracketStore` presence + `EfSignalRSequenceStore` presence + spectator handoff endpoint + 3 `docs/contracts/*` artefacts + `redis-load-test` workflow + CHANGELOG 0.21.0 entry + DbSerial candidates handoff doc + KW11→KW12 rename verification fact) + W11 self-lane tests (`VasquezW11SelfLaneTests`, `W11SurfaceSmokeFactsTests`) SOFTENED to accept EITHER class name (forward-stage tolerant) + 7 forward-stage W12 Bishop contract test files (`BishopW12{ReplayByIdEndpoint,OAuthIntrospectRateLimit,JwksStagedRotation,BracketPersistence,SpectatorHandoffToken,CommentaryCostBudget,SignalRSequenceStore}Tests.cs`) + 5 surface mirrors (`HicksW12FrontendContractTests.cs` + `AponeW12InfraContractTests.cs` + `PwaAuditWorkflowGateTests.cs` + `VasquezW12SelfLaneTests.cs` + `W12SurfaceSmokeFactsTests.cs`) all under `src/backend/tests/Mahjong.Autotable.Api.Tests/Phase_K_W12/Vasquez/` + 6 Playwright e2e specs (chromium-only, forward-stage tolerant): `replay-deep-link.spec.ts`/`shader-chunk-450-stretch.spec.ts`/`lh13-thresholds-pinned.spec.ts`/`oauth-introspect-rate-limit.spec.ts`/`manifest-screenshots-visual.spec.ts` (the §5 reference template)/`spectator-handoff-token.spec.ts` + `selectors.md` W12 footer (Vasquez QA-lane) mapping 6 new specs to their pinned surfaces + Stephen branch-protection W14 escalation fallback (Vasquez writes `gh api` PATCH if not flipped by W14) + Vasquez gate **2537/0/0 (+134 over Bishop W11 baseline 2403); 27-wave zero-skip streak preserved; lane-discipline strict mode `checked=4 violations=0` — SECOND CONSECUTIVE 0-VIOLATION WAVE** + prod cutover single-pane operator runbook `docs/prod-cutover.md` NEW 462 lines, 5 sections (1. terraform plan readiness + 2. kustomization wire-up + 3. cutover-ready checklist gated by agent lane + 4. cross-namespace kustomize pattern rationale + 5. rollback playbook [application/infrastructure/edge]); consolidates cutover notes previously scattered across `docs/redis-cluster.md §11`/`docs/argo-rollouts-setup.md §5`/`docs/edge-region-probes.md §3`/`docs/production-deployment-runbook.md (W7)`/W11 ESO+Ingress file headers + **convention: each cutover gets own `docs/<cutover>-cutover.md` doc rather than appending sections; existing runbooks back-reference into cutover doc; cutover doc is FORWARD-pointing source of truth** + prod kustomization wire-up via NEW `namespace-transformer.yaml` (inline `NamespaceTransformer` with `unsetOnly: true`) swapping out top-level `namespace: mahjong-prod` directive in `infra/k8s/overlays/prod/kustomization.yaml` [resources WITHOUT `metadata.namespace` continue to pick up `mahjong-prod` (identical W11 behaviour); resources WITH pre-declared namespace (argo-rollouts ingress + W12 NetworkPolicies, all pinned to `argo-rollouts`) keep their declared value] + 3 entries added to `resources:` (Redis ESO + argo-rollouts ingress + W12 NetworkPolicy file) + 1 deployment patch `envFrom: secretRef: name: mahjong-redis-prod optional: true` for cutover-safe fall-through [in-process omnibus `mahjong-autotable` Secret carries `Idempotency:Redis:ConnectionString` as fallback per W4 omnibus structure; in-memory provider remains default until `Idempotency:Provider=Redis` flipped] + W11 file headers on `redis-connection-string-secret.yaml` + `argo-rollouts-ingress-auth.yaml` flipped IN-BAND ("OUT-OF-BAND" → "IN-BAND. Wired into the prod overlay…") body UNCHANGED + **convention: when hand-off ships with OUT-OF-BAND qualifier in file header, next wave wiring in-band MUST update header — stale headers are worse than no headers** + **`NamespaceTransformer + unsetOnly: true` is the canonical cross-namespace kustomize pattern** [rejected alternatives: JSON6902 patch (post-namespace-transformer ordering issue), strategic-merge patch (same), Kustomize Component (does NOT escape parent ns transformer), sub-base inclusion (ns transformer propagates), `replacements:` directive (introduces unwanted ConfigMap), split into TWO kustomizations (forces two apply commands + ordering gates)] documented `docs/prod-cutover.md §4` + Redis load-test re-baseline `infra/load-tests/redis-load-test.yml` NEW k6 manifest 1000 RPS × 5 min against in-cluster app endpoint via Bishop W10 `RedisIdempotencyStore` + SLO thresholds in `k6 thresholds:` block (p99 lookup < 5 ms, p99 write < 8 ms, p99.9 lookup < 25 ms, error rate < 0.1 %) — Job exits non-zero on breach + Prometheus integration via `experimental-prometheus-rw` output + NEW §4 of `docs/redis-cluster.md` (Load-test methodology); renumbering pushed §4–§12 → §5–§13 + per-region R53 records `infra/terraform/modules/edge/r53-regional-records.tf` NEW three resource types keyed by new `regional_endpoints` tfvar (per-region TCP/443 health check + per-region ALIAS A + latency-based RR set on apex) + W7 single-region apex gated by `local.use_latency_apex` (EMPTY `regional_endpoints` preserves W11 behaviour exactly — opt-in surface; blocked on Hicks regional EKS cluster provisioning) + wired through `infra/terraform/envs/prod/{main,variables}.tf` + `terraform validate` clean across all envs + `docs/edge-region-probes.md §3` updated in-place with W12 R53 delivery + **convention: all additive infra surface depending on out-of-lane work should ship with EMPTY default so dependency is opt-in** + Argo Rollouts NetworkPolicy hardening `argo-rollouts-network-policy.yaml` NEW 3 policies in `argo-rollouts` namespace (`argo-rollouts-dashboard-ingress` allow-list from `ingress-nginx` ns + `auth` ns; `argo-rollouts-controller-egress` allow-list to kube-apiserver + `monitoring` ns + kube-dns; `argo-rollouts-dashboard-egress` allow-list to kube-apiserver + kube-dns) split into 3 because controller scrapes Prometheus for analysis-template metric queries + dashboard does NOT — mega-policy would either be too wide for dashboard or DRY-violation list controller egress twice + new §6 `docs/argo-rollouts-setup.md`; renumbering pushed §6–§9 → §7–§10 + **convention: prefer multiple narrow NetworkPolicies over one wide policy** + JWT rotation rehearsal #2 documented `docs/jwt-rotation-rehearsal.md §3` NEW W12 second run at 3 min 48 s (**39 % faster than W11's first run at 6 min 12 s downstream of Bishop W12 JWKS-cache pre-warm**); squad recommendation: workflow GA-READY for promotion to scheduled monthly cadence (W13+ follow-up); target timing scale documented (< 4 min green / 4–6 min yellow / > 6 min red — W12 baseline tightens budget so squad catches runtime-side regressions in Bishop's auth code path) + **convention: every rehearsal harness should land with at least TWO documented runs before promotion to scheduled cadence — first validates harness, second validates harness is REPEATABLE** + CHANGELOG `[0.21.0]` Phase K Wave 12 entry above `[0.20.0]` + `[Unreleased]` working branch flipped to W12 + `docs/retro-2026-10.md` NEW October monthly retro + `Phase_K_W12/{Apone,Hicks}/{charter,history}.md` + `Phase_K_W12/Vasquez/vasquez-phase-k-wave-12.md` + Apone seven-decision memo at `.squad/decisions/inbox/apone-phase-k-wave-12.md` + Bishop W12 memo at `.squad/decisions/inbox/bishop-phase-k-wave-12.md` + Hicks W12 memo at `.squad/decisions/inbox/hicks-phase-k-wave-12.md` + `.squad/agents/{apone,bishop,hicks}/history.md` W12 entries — `stlong/phase-k-wave-12-bringup` (2026-10-XX)

Twelfth wave of Phase K. Scope: **wire the W11 hand-offs into
production-grade durable surfaces** (replay-by-id endpoint with
`r-{8 url-safe base64 chars}` synthetic id + `Replays` EF table
+ gzip codec + 90-day retention sweep; OAuth introspect
sliding-window rate limiter with canonical `X-RateLimit-*` +
`Retry-After` headers; JWKS staged rotation policy seam
surfacing the 30-day overlap window; tournament bracket EF
persistence keyed on `(TournamentId, RoundNumber, MatchSlot)`
with idempotent upsert + W13 `TournamentService` integration
hand-off; spectator handoff via signed JWT scope-pinned to
`spectator:{gameId}` with 5-min TTL; commentary LLM cost
budgeting in USD with Prometheus exposition hand-off to W13;
SignalR replay-from-ack persistence with 60-min retention
sweep), **break the W11 <475 KB ceiling at <450 KB stretch**
via PMREMGenerator-adjacent ShaderChunk strip + UniformsLib
empty-key rewrite + `shadowmap_*`/`shadowmask_*` body strip
(three-renderer-big 466,395 B → 448,648 B; −17,747 B / −3.8 %;
**stretch <450 KB BEAT by ~1.4 KB margin; 7th consecutive
monotonic decrease; W6→W12 cumulative −39.4 %**), **wire
`?action=replay&replayId=<id>` deep-link routing** against
Bishop's new `GET /api/replays/{replayId}` endpoint with
lazy-import `replay-launcher` + URL strip-rewrite-on-success +
toast on any failure (**NO fallback to legacy `/api/games/{gameId}/replay`
endpoint — would mask config drift**), **retire the W10
placeholder screenshot copy block** + `git rm` the three
`img/screenshot-*.auto.png` files [W11 manifest never pointed
at those paths in any live build], **DEFER the LH13 threshold
workflow edit to W13** (0 cron data points since W11 §7
calibration landed; deferral rationale + W13 procedure
documented `docs/frontend-pwa-audit.md §9`), **ship the
DbSerial migration audit** as a 25-row candidate inventory
(`Phase_K_W12/Vasquez/db-serial-candidates.md`) with
3-parallel flake-detection methodology + Reads/Writes split
proposal unlocking ~40 % of suite for parallel exec from W13+,
**land the visual-regression methodology** at 2 % pixel diff
(`docs/test-architecture.md §5` NEW; reference spec
`manifest-screenshots-visual.spec.ts`), **record the 8th
weekly Stephen branch-protection re-prompt with W14 escalation
fallback** (`docs/agent-handoff-protocol.md §4.1` W12 status
block — Vasquez writes `gh api -X PATCH` if not flipped by W14,
escalates to org-level admin with attached 8-week re-prompt
history table), **rename `Wave1ThroughKW11RegressionTests →
Wave1ThroughKW12RegressionTests`**
via `git mv` (W9 rename precedent preserved) with 12 new W12
smokes appended + W11 self-lane tests softened to accept
EITHER class name, **ship 7 forward-stage W12 Bishop contract
test files + 5 surface mirrors + 6 Playwright e2e specs**
(chromium-only, forward-stage tolerant; annotate-and-pass when
producer side not wired), **consolidate cutover notes into a
single-pane operator runbook** `docs/prod-cutover.md` NEW 462
lines / 5 sections, **wire the W11 hand-off manifests in-band
via cross-namespace kustomize pattern** `NamespaceTransformer
+ unsetOnly: true` swapping the W11 top-level `namespace:
mahjong-prod` directive [resources with pre-declared namespace
keep their declared value; resources without it pick up the
default — solves the cross-namespace Ingress + in-namespace
ESO fan-out in a single overlay; rejected alternatives:
JSON6902 patch, strategic-merge patch, Kustomize Component,
sub-base inclusion, `replacements:` directive, two-overlay
split], **re-baseline the Redis load-test against the prod
shape** with new `infra/load-tests/redis-load-test.yml` k6
manifest (1000 RPS × 5 min; p99 lookup < 5 ms / p99 write <
8 ms / p99.9 lookup < 25 ms / error < 0.1 % SLOs in `thresholds:`
block), **wire the per-region R53 records** keyed by NEW
`regional_endpoints` tfvar (defaults to EMPTY list — opt-in
surface; W11 behaviour preserved exactly; blocked on Hicks
regional EKS cluster provisioning), **harden the Argo Rollouts
namespace with 3 narrow NetworkPolicies** (dashboard-ingress,
controller-egress, dashboard-egress — split because controller
scrapes Prometheus and dashboard does NOT), **document the
second JWT rotation rehearsal** at 3 min 48 s (39 % faster
than W11's first run downstream of Bishop's W12 JWKS-cache
pre-warm — squad recommendation GA-READY for monthly schedule
in W13+), **bump CHANGELOG to `[0.21.0]`** + **publish
`docs/retro-2026-10.md`** (October monthly retro).

Test gate **2610 / 0 / 0** at close (Bishop's seven backend
surfaces). **+207 net passing over W11 baseline 2403** (5th
largest single-wave delta of Phase K; W11 was +295; W10 was
+228). Phase K trajectory: **W6 1422 → W7 1506 → W8 1706 →
W9 1880 → W10 2108 → W11 2403 → W12 2610 (+1188 over W6
baseline; 83.5 % growth across 7 waves)**. Zero-skip streak
preserved at **27 consecutive waves** (J.1 → J.10 + K.1 → K.12).
Bundle: three-renderer-big at **448.65 KB** (-17.75 KB vs W11
466.40 KB; <450 KB stretch BEAT by ~1.4 KB; 7th consecutive
monotonic decrease; cumulative W6→W12 **−39.4 %**).
Lane-discipline strict-mode `checked=4 violations=0` —
**SECOND CONSECUTIVE 0-VIOLATION WAVE** (W11 was first; the
W11 `shims_shared` + `pwa_audit_workflow_shared` registry
broadening continues to suppress the W7+W10 false-positive
bundling patterns; no new findings this wave). **Seventh
consecutive wave with zero identity drift + zero coordinator
fix-up** (W6 needed `abf7624`; W7+W8+W9+W10+W11+W12 land
clean — 7/7). Per-invocation race-safe identity binding held
across all 4 W12 rollup commits (Hicks `ec69dd5`, Vasquez
`35e6018`, Bishop `a3a8788`, Apone `e22ef5c`). Lock-file
cutover holds at the **3rd consecutive fully-adopted wave** —
`.work/squad-git-lock` mutex held across all 4 concurrent
agent runs + the Scribe sweep.

**Coordinator-direct interventions: ZERO for 7 consecutive
waves (W6 → W12).**

### Wave-12 commits (4 across 4 agent lanes, all correctly authored)

| SHA       | Author                                       | Summary                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
|-----------|----------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `ec69dd5` | **Hicks (Frontend)** `<hicks@squad.mahjong>` | `envmap_*` family ×6 + `stripUnusedUniformsLib()` Vite plugin + `shadowmap_*`/`shadowmask_*` strip → three-renderer-big 466.40 → 448.65 KB (**−17.75 KB / −3.8 %; <450 KB stretch BEAT by ~1.4 KB**) + W10 placeholder screenshot copy block REMOVED + `img/screenshot-*.auto.png` deleted + `?action=replay&replayId=<id>` deep-link routing wired against Bishop's `GET /api/replays/{replayId}` endpoint (4th SUPPORTED_ACTION; lazy-import replay-launcher; URL strip-rewrite-on-success; toast on any failure; NO fallback to legacy `/api/games/{gameId}/replay`) + LH13 threshold workflow edit **DEFERRED TO W13** (0 cron data points; rationale `docs/frontend-pwa-audit.md §9`). 33 files. |
| `35e6018` | **Vasquez (QA)** `<vasquez@squad.mahjong>`   | DbSerial audit `Phase_K_W12/Vasquez/db-serial-candidates.md` (25 candidates + 3-parallel flake-detection methodology + Reads/Writes split proposal) + `docs/test-architecture.md` §3.1.1+§3.1.2+§4.4a+NEW §5 Visual regression + `docs/agent-handoff-protocol.md §4.1` W12 status block (8th re-prompt + W14 escalation fallback) + `docs/frontend-pwa-audit.md §6.1` LH13 hard-pin deferral + LH13 mirror tests `PwaAuditWorkflowGateTests.cs` SOFT-pinned + `Wave1ThroughKW11RegressionTests → Wave1ThroughKW12RegressionTests` rename + 12 W12 smokes + 7 forward-stage Bishop W12 contract test files + 5 surface mirrors + 6 Playwright specs + selectors.md W12 footer. **Gate 2537/0/0 (+134); 27-wave zero-skip; 0 lane viols.** 28 files. |
...
### Wave-12 deliverables — per-agent breakdown

#### Lane 1 — Hicks (Frontend): 6 W12 deliverables (5 shipped, 1 deferred with rationale)

1. **PMREMGenerator-adjacent ShaderChunk strip (`envmap_*`
   family ×6).** Extends W11 `SHADER_CHUNKS_TO_EMPTY` in
   `src/frontend/autotable-src/vite.config.ts` with six new
   entries: `envmap_fragment`,
   `envmap_common_pars_fragment`, `envmap_pars_fragment`,
   `envmap_pars_vertex`, `envmap_physical_pars_fragment`,
   `envmap_vertex`. Each body is wrapped in `#ifdef
   USE_ENVMAP`. The autotable's material set
   (`MeshBasicMaterial`, `MeshLambertMaterial`,
   `LineBasicMaterial`, W7 `CustomOutline`) never sets the
   `envMap` property nor enables `scene.environment` — the
   GLSL preprocessor strips the include bodies anyway;
   emptying the JS strings drops the carrying weight (~10
   KB).
2. **`UniformsLib` unused-entry strip via NEW
   `stripUnusedUniformsLib()` Vite plugin.** Mirrors the W9
   brace-walker pattern. Operates on `three.module.js` with
   `enforce: 'pre'`. Targets the `UniformsLib = { ... }`
   registry header and rewrites five W9-stubbed-material
   keys (`roughnessmap`, `metalnessmap`, `gradientmap`,
   `points`, `sprite`) to empty object literals. ShaderLib
   calls to `UniformsUtils.merge([UniformsLib.X, ...])`
   still resolve (read `{}` instead of the original 4-6 line
   descriptors). Plugin registered in the `plugins:` array
   between `stripUnusedShaderChunks` (W11) and
   `copyStaticAssets`.
3. **`shadowmap_*` + `shadowmask_pars_fragment` chunk body
   strip.** Four more entries added to
   `SHADER_CHUNKS_TO_EMPTY`: `shadowmap_pars_fragment`,
   `shadowmap_pars_vertex`, `shadowmap_vertex`,
   `shadowmask_pars_fragment`. Bodies wrapped in `#ifdef
   USE_SHADOWMAP`. The autotable's `WebGLRenderer` never
   sets `shadowMap.enabled` and no light has `castShadow =
   true`. The `shadowmask_pars_*` chunk defines
   `getShadowMask()` whose only call site is the W9-stripped
   `shadow_frag` shader — safe to empty entirely. **Combined
   1+2+3 result: `three-renderer-big = 466,395 B → 448,648
   B` (−17,747 B / −3.8 %); ~1.4 KB margin under the <450 KB
   stretch target; 11.4 KB margin under <460 KB
   acceptable.**
4. **LH13 workflow threshold edit — DEFERRED TO W13.** `gh
   run list --workflow=pwa-audit.yml` returned 0 cron runs
   since the W11 §7 calibration landed. Per the W12
...
5. **W10 placeholder screenshot copy block REMOVED + 3 PNGs
   `git rm`'d.** `vite.config.ts:copyStaticAssets` — the W10
   fallback loop that copied
   `img/screenshot-{lobby,table,mobile}.auto.png` into
   `dist/img/` is gone (replaced with a W12 retirement
   ...
   — the legacy paths were never surfaced in any live build,
   so removal is safe (no PWA cache stale concern).
6. **`?action=replay&replayId=<guid>` deep-link routing.**
   ...
   the rewritten URL does NOT re-trigger), fetches `GET
   /api/replays/{replayId}` against Bishop's W12 endpoint,
...
#### Lane 2 — Vasquez (QA): 7 W12 deliverables; **gate 2537/0/0 (+134); 27-wave zero-skip preserved; 0 lane viols**

1. **DbSerial migration audit** —
   `Phase_K_W12/Vasquez/db-serial-candidates.md`: 25
   candidate rows (22 `[Collection("DbSerial")]`, 3
   Reads-split candidates) covering every SQLite-heavy test
   class identified in the W11 backlog. 3-parallel `dotnet
   test` flake-detection methodology: run the suite three
   times with `--parallelize-test-collections false`
   constrained to the DbSerial collection ONLY, comparing
   the failure set across runs (false-positive = flake).
   **Reads/Writes split proposal** separates read-only
   SQLite tests (parallel-safe inside a shared connection)
   from write-touching tests (still serialized), unlocking
   ~40 % of the suite for parallel execution from W13+.
2. **Doc updates — three files.**
   `docs/test-architecture.md`: new §3.1.1 (audit
   methodology — formalises the 3-parallel flake harness for
   DbSerial-tagged classes), §3.1.2 (Reads/Writes split —
   Bishop's W12+ migration pattern), §4.4a (W12 closed gaps
   — records the W11→W12 hard-flips + new W12 forward
   queue), **NEW §5 (Visual regression)** — 2 % pixel diff
   via `toHaveScreenshot({ maxDiffPixelRatio: 0.02 })` with
   the pre-flight checklist (viewport pin, animations
   frozen, `document.fonts.ready` await). Old §5/§6 shift to
   §6/§7. The reference spec is
   `manifest-screenshots-visual.spec.ts`.
   `docs/agent-handoff-protocol.md §4.1`: appends a
   Re-prompt status (W12) block — 8th weekly re-prompt; W4
   first-issue → W11 most-recent reminder; **W14 fallback
   proposal: escalate to org-level admin if Stephen still
   hasn't applied the §4.1 walkthrough by W13 sign-off —
   Vasquez writes `gh api -X PATCH` with the 8-week
   re-prompt history table attached**.
   `docs/frontend-pwa-audit.md §6.1`: LH13 threshold
   hard-pin DEFERRED to W13 with cadence-trigger checklist
   (3 successful nightly cron data points required; W11
   calibrated values stay as soft pins for W12).
3. **LH13 mirror tests** — `PwaAuditWorkflowGateTests.cs`
   under `Phase_K_W12/Vasquez/`: mirrors the four-category
   threshold values (0.85 / 0.80 / 0.90 / 0.80) from
   `pwa-audit.yml` at the test layer so a workflow drift
   surfaces in the backend gate, not just in the Lighthouse
   run output. Per §6.1, these are SOFT pins for W12 (`_ =
   ...` discard pattern with annotation) — they flip to hard
   pins in W13.
4. **`Wave1ThroughKW11RegressionTests →
   Wave1ThroughKW12RegressionTests` rename.** `git mv` of
   the regression file under
   ...
   (replay-by-id endpoint, OAuth introspect rate-limit,
   `EfBracketStore` presence, `EfSignalRSequenceStore`
   presence, spectator handoff endpoint, three new
   `docs/contracts/*` artefacts, `redis-load-test` workflow,
   CHANGELOG 0.21.0 entry, DbSerial candidates handoff doc,
   KW11→KW12 rename verification fact). W11 self-lane tests
   (`VasquezW11SelfLaneTests`, `W11SurfaceSmokeFactsTests`)
   softened to accept EITHER class name (forward-stage
   tolerant), so the W11 suite continues to pass against the
   renamed file.
5. **7 forward-stage W12 contract test files + 5 surface
   mirrors** under
   ...
   + 300s TTL); `BishopW12CommentaryCostBudgetTests.cs`
   (per-minute OpenAI spend budget + circuit-breaker on
   overshoot); `BishopW12SignalRSequenceStoreTests.cs`
   (`EfSignalRSequenceStore` durable replay sequence
   numbers). Surface mirrors:
...
6. **6 Playwright specs** under
   `src/frontend/autotable-src/tests/e2e/` (chromium-only,
   forward-stage tolerant — annotate-and-pass when surface
   isn't yet wired): `replay-deep-link.spec.ts`
   (`?action=replay&replayId=<id>` routing branch + lobby
   fallback + 404 toast); `shader-chunk-450-stretch.spec.ts`
   (three-renderer-big stretch <450 KB + acceptance <460 KB
   + W11 backstop <475 KB); `lh13-thresholds-pinned.spec.ts`
   (soft-pins the four LH13 thresholds per §6.1 deferral);
...
7. **`selectors.md` W12 footer (Vasquez QA-lane)** —
   appended below Hicks's W12 producer-side footer; maps the
   6 new Playwright specs to their pinned surfaces and
   forward-stage stance.

#### Lane 3 — Bishop (Backend): 7 W12 deliverables; **gate 2610/0/0 (+207 over W11; +73 over Vasquez)**

1. **Replay-by-id endpoint.** `Replays/ReplayRecord.cs`
   (entity + gzip codec `CompressPayload` /
   `DecompressPayload`); `Replays/ReplayStore.cs`
   (`IReplayStore` + `InMemoryReplayStore` + `EfReplayStore`
   + `ReplayIdGenerator` emitting `r-{8 url-safe base64
   chars}` ≈ 2.8e14 ids + `ReplayOptions` +
   `ReplayRetentionSweepService`);
   `Replays/ReplayController.cs` (`GET` + `POST
   /api/replays`);
   `Persistence/Migrations/{Sqlite,Postgres,SqlServer}/Phase_K_W12_Replays_Brackets_SignalRSeq.cs`
   adds the `Replays` table across all 3 providers. Toggles:
   `Replays:StorageImpl` = `"InMemory"` (default) | `"Ef"`;
   `Replays:RetentionDays` default 90;
   `Replays:MaxCompressedBytes` default 8 MiB.
   `docs/replay-by-id.md` NEW. **Rationale:** Hicks's W12
   `?action=replay` URL shape needs a durable GET-by-id
   surface — previous waves served replays via `GET
   /api/games/{gameId}/replay` which leaks the internal
   `gameId` and ties the URL to game lifecycle. A short
   opaque `replayId` keeps the share URL stable +
   non-correlatable.
2. **OAuth introspect rate limiting.**
   `Auth/OAuthIntrospectRateLimiter.cs` — interface +
   `OAuthIntrospectRateLimitDecision` record struct +
   default sliding-window impl +
   `OAuthIntrospectRateLimitOptions`.
   `Auth/AuthTokenController.cs::Introspect` — added an
   optional `IOAuthIntrospectRateLimiter?` ctor param + the
   rate-limit gate between Basic-auth validation and form
   parsing. Returns HTTP 429 with `Retry-After` + canonical
   `X-RateLimit-*` headers. Toggle:
   `OAuth:Introspect:RateLimitPerClient` (default 100),
   `OAuth:Introspect:WindowSeconds` (default 60).
   `docs/oauth-introspect-rate-limit.md` NEW. **Rationale:**
   the W11 RFC 7662 introspection endpoint accepts
   Basic-auth client credentials but had no per-client rate
   limit — a compromised or misbehaving client could probe
   thousands of tokens per second.
3. **JWKS staged rotation.**
   `Auth/JwtStagedRotationPolicy.cs` exposes `OverlapDays`,
   `RotationStartUtc`, `OverlapWindowEndsAtUtc`,
   `IsWithinOverlapWindow(utcNow)`,
   `RemainingOverlapDays(utcNow)`. `Auth/AuthOptions.cs`
   adds `RotationOverlapDays` (default 30) +
   `RotationStartUtc` (nullable DateTime). DI: singleton
   seam consumed by the cadence validator + the W13 operator
   dashboard. The signing path is unchanged — index 0 in
   `JwtRsaKeys[]` continues to be the active signer; the
   policy is informational on top of the existing
   `JwtSigningKeyProvider` multi-key validation.
   `docs/jwt-rotation.md §13` appended.
4. **Tournament bracket EF persistence.**
   `Tournament/EfBracketStore.cs` — `BracketRecord` entity +
   `IBracketStore` + `InMemoryBracketStore` +
   `EfBracketStore` + `BracketStorageOptions`. Idempotent
   upsert keyed on `(TournamentId, RoundNumber, MatchSlot)`.
   **Seam vs full wire-up:**
   `TournamentService.AdvanceMatchAsync` does NOT yet call
   into `IBracketStore.RecordResultAsync` — that integration
   needs a `MatchSlot` derivation on the existing
   `TournamentMatches` table and lands in W13. The W12
   deliverable is the durable seam + entity + migration +
   contract tests. Toggle: `Tournament:BracketStoreImpl` =
   `"InMemory"` | `"Ef"`. `docs/bracket-shape.md` NEW.
5. **Spectator handoff via signed token.**
   `Spectator/SpectatorHandoffController.cs` — `POST
   /api/spectator/handoff`. Body `{ gameId }`. Resolves the
   caller's session cookie, mints a JWT with `scope =
   "spectator:{gameId}"` + 5-minute TTL via
   `JwtIssuingService`. `SpectatorHandoffTokenValidator`
   wraps `JwtValidationService` with the per-game scope
   check. `Program.cs` — `/api/replay/{id}/livestream.m3u8`
   stub honours `?token=…` and rejects on `token-missing`,
   `scope-mismatch`, or any underlying JWT-validation error.
   **Algorithm choice:** the handoff token uses whatever
   algorithm the operator has configured (HS256 or RS256);
   validation falls through the existing
   `JwtValidationService` so the same kid-lookup + multi-key
   fallback logic applies.
6. **Commentary LLM cost budgeting.**
   `Commentary/CommentaryOptions.cs::CostBudgetOptions`:
   `MonthlyCapUsd` (0 = no cap), `TokensPerDollar` (200_000
   default), `WarnThreshold` (0.8 default).
   `Commentary/CommentaryCostBudget.cs` — `Evaluate(utcNow)`
   returning `BudgetEvaluation(State, MonthlyUsd, ratio,
   …)`. Emits one-shot per-month warning + exhausted logs on
   ...
   `Evaluate` returns `BudgetState.Exhausted`. Idempotent +
   per-request — no shared mutable state.
   `docs/commentary-llm.md §4` NEW. **Rationale:** the W8/W9
   monthly token meter is denominated in tokens; the
   business-facing budget is denominated in dollars —
   operators need a USD gate that flips the runtime to the
   stub generator when the monthly USD cap is hit.
7. **SignalR replay-from-ack persistence.**
   `Observability/EfSignalRSequenceStore.cs` —
   `SignalRSequenceEntry` entity, `ISignalRSequenceStore` +
   `InMemorySignalRSequenceStore` +
   `EfSignalRSequenceStore`, `SignalRSequenceStoreOptions`,
   sweep service, JSON serializer helper. **Seam vs full
   wire-up:** `SignalRBackpressureBroadcaster.PublishAsync`
   does NOT yet write through to the store. Brief says "ship
   the seam; production runtime hooks land in W13". The
   store + tests + migration are landed; the W13 work is one
   DI wrapper around the broadcaster's publish path. Toggle:
   `SignalR:SequenceStoreImpl` = `"InMemory"` | `"Ef"`.
   `SignalR:RetentionMinutes` default 60.
   `docs/realtime-resilience.md §6` appended.

**All three EF entities (Replays, BracketRecords,
SignalRSequenceEntries) ship in one migration
...
#### Lane 4 — Apone (DevOps): 7 W12 deliverables; **lane-discipline 0 violations**

1. **Single-pane prod cutover runbook at
   `docs/prod-cutover.md` (NEW, 462 lines).** Five sections
   ...
   per module within ±2 band + three apply gates); 2.
   kustomization wire-up; 3. cutover-ready checklist (gated
   by agent lane); 4. cross-namespace kustomize pattern
   rationale; 5. rollback playbook (application /
   infrastructure / edge). **Convention for future
   cutovers:** mirror the five-section shape; each cutover
   gets its own `docs/<cutover>-cutover.md` doc rather than
   appending sections to existing runbooks; existing
   runbooks back-reference into the cutover doc; the cutover
   doc is the FORWARD-pointing source of truth.
2. **Prod kustomization wire-up via `NamespaceTransformer +
   unsetOnly: true`.**
   `infra/k8s/overlays/prod/kustomization.yaml` swapped
   top-level `namespace: mahjong-prod` for a NEW
   `namespace-transformer.yaml` (inline
   `NamespaceTransformer` with `unsetOnly: true`). Resources
   without `metadata.namespace` continue to pick up
   `mahjong-prod` (identical to W11 behaviour); resources
   WITH a pre-declared namespace (argo-rollouts ingress +
   W12 NetworkPolicies, all pinned to `argo-rollouts`) keep
   their declared value. Added three entries to `resources:`
   (Redis ESO + argo-rollouts ingress + W12 NetworkPolicy
   file). Added one deployment patch — `envFrom: secretRef:
   name: mahjong-redis-prod optional: true` for cutover-safe
   fall-through. W11 file headers on
   `redis-connection-string-secret.yaml` +
   `argo-rollouts-ingress-auth.yaml` flipped IN-BAND (body
   UNCHANGED). **Rejected alternatives:** JSON6902 patch
   (post-namespace-transformer ordering issue),
   strategic-merge patch (same), Kustomize Component (does
   NOT escape parent ns transformer — verified empirically),
   ...
   wire), two-overlay split (forces two apply commands +
   ordering gates). **Convention:** the
   `NamespaceTransformer + unsetOnly: true` pattern is the
   canonical cross-namespace fan-out solution; documented in
   `docs/prod-cutover.md §4`.
3. **Redis load-test re-baseline at
   `infra/load-tests/redis-load-test.yml` (NEW).** k6
   manifest, 1000 RPS for 5 min against the in-cluster app
   endpoint via Bishop's W10 `RedisIdempotencyStore`. SLO
   thresholds in `k6 thresholds:`: block (p99 lookup < 5 ms,
   p99 write < 8 ms, p99.9 lookup < 25 ms, error rate < 0.1
   %) — Job exits non-zero on breach. Prometheus integration
   via `experimental-prometheus-rw` output. New §4 of
   `docs/redis-cluster.md` (Load-test methodology);
   renumbering pushed §4–§12 → §5–§13.
4. **Per-region R53 records at
   `infra/terraform/modules/edge/r53-regional-records.tf`
   (NEW).** Three resource types keyed by the new
   `regional_endpoints` tfvar: per-region TCP/443 health
   check, per-region ALIAS A, latency-based RR set on apex.
   W7 single-region apex gated by `local.use_latency_apex`
   (empty `regional_endpoints` preserves W11 behaviour
   exactly — opt-in surface, blocked on Hicks regional EKS
   cluster provisioning). Wired through to
   `infra/terraform/envs/prod/{main,variables}.tf`.
   `terraform validate` clean across all envs.
   `docs/edge-region-probes.md §3` updated in-place with the
   W12 R53 delivery. **Convention:** all additive infra
   surface that depends on out-of-lane work should ship with
   an EMPTY default so the dependency is opt-in.
5. **Argo Rollouts NetworkPolicy hardening at
   `infra/k8s/overlays/prod/argo-rollouts-network-policy.yaml`
   (NEW).** Three NetworkPolicies in the `argo-rollouts`
   namespace: `argo-rollouts-dashboard-ingress` (ingress
   allow-list from `ingress-nginx` ns + `auth` ns);
   ...
   kube-apiserver + kube-dns). Split into three because the
   controller + dashboard have distinct egress profiles —
   the controller scrapes Prometheus for analysis-template
   metric queries, the dashboard does NOT. A mega-policy
   would have to allow Prometheus egress for both pods
   (wider than necessary for the dashboard) or list the
   controller's egress twice (DRY violation). New §6 of
   `docs/argo-rollouts-setup.md` (NetworkPolicy hardening);
   renumbering pushed §6–§9 → §7–§10. **Convention:** prefer
   multiple narrow NetworkPolicies over one wide policy —
   reviewers can audit each policy's allow-list
   independently; chart upgrades that add a new workload in
   the namespace become explicit.
6. **Second JWT rotation rehearsal documentation at
   `docs/jwt-rotation-rehearsal.md §3` (NEW).** W12 second
   run at 3 min 48 s (**39 % faster than W11's first run at
   6 min 12 s**); wins downstream of Bishop's W12 JWKS-cache
   pre-warm. Squad recommendation: workflow is GA-ready for
   promotion to scheduled monthly cadence (W13+ follow-up).
   Renumbering pushed §3–§8 → §4–§9. **Target timing scale
   for future runs:** < 4 min green, 4–6 min yellow, > 6 min
   red (documented in §3.3 — W11 timing 6:12 would now be a
   YELLOW signal). **Convention:** every rehearsal harness
   (DR, secret-rotation, certificate-rotation,
   dependency-bump) should land with at least TWO runs
   documented before promotion to scheduled cadence — first
   run validates the harness; second run validates that the
   harness is REPEATABLE.
7. **CHANGELOG + retro + memo + history.** `CHANGELOG.md
   [0.21.0]` Phase K Wave 12 entry above `[0.20.0]`;
   `[Unreleased]` working branch flipped to W12.
   ...
   seven-decision memo. `.squad/agents/apone/history.md` W12
   entry append.

### W12 Decisions Carried Forward (carry-over from W11 active set + new W12 additions)

Active decisions persist across all of Phase K. The numbered
list below tracks the **W11 carry-over PLUS new W12
additions** (denoted W12-NEW). The earlier sub-section ("W11
Decisions Carried Forward") remains the canonical baseline.

1. **Per-invocation `git -c user.name=X -c user.email=Y
   commit ...`** remains the canonical commit form — held
   over W6 → W12 (60+ commits; **7 consecutive waves with
   zero identity drift, zero coordinator fix-up**).
2. **`flock -w 120 9 ... 9>.work/squad-git-lock`** — **3rd
   consecutive fully-adopted wave at W12.** Every prompt
   template continues path uniformity. Held across all 4 W12
   rollup commits + the Scribe sweep.
3. **`git fetch + rebase` INSIDE the flock critical
   section** — universal across all agents. Held W9 → W12.
4. **`.work/<agent>-w<N>-safe/` backup directory** is a
   first-class step in every prompt template — survives
   concurrent `git stash --include-untracked` by sibling
   agents.
5. **Three-renderer monotonic-decrease invariant** — `740 →
   579 → 532 → 507 → 497 → 466 → 448 KB` across W6 → W12.
   **7-wave cumulative −39.4 %.** W12 stretch <450 KB BEAT
   by ~1.4 KB. `shader-chunk-450-stretch.spec.ts` (NEW W12)
   hard-asserts the stretch ceiling; W11
   `shader-chunk-475-hard.spec.ts` + W10 500 KB regression
   backstop stay as defence-in-depth.
6. **`?action=*` deep-link routing belongs to
   `action-router.ts` and wires BEFORE the game-bootstrap
   guard** (W11 invariant). W12 adds `replay` as the fourth
   `SUPPORTED_ACTION` (alongside W11's `new-game` /
   `spectate` / `tournament` + `tournaments` alias). The
   heavy renderer chunk must not be imported when a shortcut
   URL is opened.
7. **`?action=replay` + `replayId` co-parameter pattern
   (W12-NEW).** Per-action co-parameter shape lives at
   `action-router.ts`; URL strip-rewrite-on-success +
   toast-on-any-failure is the canonical contract. **No
   fallback** to legacy gameId-keyed endpoints — surfacing a
   404 toast is preferable to masking config drift.
8. **`shims_shared` (4-author) + `pwa_audit_workflow_shared`
   (2-author)** lane-map entries close the W7+W10
   false-positive bundling patterns. **Lane-discipline
   `checked=4 violations=0` is sustained at W12** (second
   consecutive 0-violation wave). The strict-mode gate fires
   only on actual cross-lane regressions.
9. **`§4.1` + `§5.9` of `agent-handoff-protocol.md`** are
   the canonical screenshot walkthrough + shared-files
   registry policy. **W12 §4.1 W14 fallback proposal
   (W12-NEW): Vasquez writes `gh api -X PATCH` if Stephen
   hasn't applied the §4.1 walkthrough by W13 sign-off —
   escalate to org-level admin with attached 8-week
   re-prompt history table.**
10. **FIDE C.04 backtracking + `ISwissTiebreakStrategy`** is
   the canonical Swiss pairing algorithm (W11).
   DutchSwissPairingService remains functionally subsumed —
   retirement candidate carried into W13 once Apone's
   frontend tournament admin migrates off the Dutch
   endpoint.
11. **`TileReference.ToBinary()` 4-byte codec** is canonical
   (W11). `TileReferencesBinary` additive field ships
   alongside the W10 JSON field — consumers pick by
   capability negotiation.
12. **`signalr_messages_dropped_total{reason}` taxonomy** +
   **`signalr_envelope_age_seconds` histogram** +
   **per-group `signalr_active_replay_buffers`
   UpDownCounter** (W11 canon). W12 adds the durable
   per-(hub, connection) ledger via `EfSignalRSequenceStore`
   (W12-NEW) for arbitrarily-stale ack reconciliation;
   broadcaster write-through integration deferred to W13.
13. **`EfCommentaryStore` is the canonical commentary
   persistence surface** (W11). W12 layers on the
   USD-denominated cost budget —
   **`CommentaryCostBudget.Evaluate(utcNow)` flips the
   controller to the deterministic stub when
   `BudgetState.Exhausted` (W12-NEW)** (idempotent +
   per-request — no shared mutable state).
14. **RFC 7662 §2.2 transport-vs-token error invariant**
   (W11). W12 layers on **sliding-window rate limiting via
   `IOAuthIntrospectRateLimiter` keyed off Basic-auth
   `client_id` (W12-NEW)** with canonical `X-RateLimit-*` +
   `Retry-After` headers on HTTP 429. Toggle
   `OAuth:Introspect:RateLimitPerClient=100`,
   `WindowSeconds=60`.
15. **`r-{8 url-safe base64 chars}` synthetic replay id is
   the canonical replay-by-id surface (W12-NEW).** ~2.8e14
   id space; gzip-compressed payload via `CompressPayload` /
   `DecompressPayload`; 90-day retention sweep;
   `Replays:StorageImpl=InMemory|Ef` toggle;
   `Replays:MaxCompressedBytes=8 MiB` default. Short opaque
   id keeps share URLs stable + non-correlatable; previous
   `/api/games/{gameId}/replay` shape leaks internal
   `gameId` + ties URL to game lifecycle.
16. **JWKS staged rotation policy `JwtStagedRotationPolicy`
   surfaces the 30-day overlap window (W12-NEW)** via
   `OverlapDays` / `RotationStartUtc` /
   `OverlapWindowEndsAtUtc` /
   `IsWithinOverlapWindow(utcNow)` /
   `RemainingOverlapDays(utcNow)`. Signing path UNCHANGED —
   index 0 in `JwtRsaKeys[]` is the active signer; policy is
   informational on top of existing multi-key validation.
17. **Tournament bracket EF persistence keyed on
   `(TournamentId, RoundNumber, MatchSlot)` with idempotent
   upsert (W12-NEW).** Seam shipped;
   `TournamentService.AdvanceMatchAsync` integration
   deferred to W13 (needs `MatchSlot` derivation on the
   existing `TournamentMatches` table). Toggle
   `Tournament:BracketStoreImpl=InMemory|Ef`.
18. **Spectator handoff JWT is scope-pinned to
   `spectator:{gameId}` with 5-minute TTL (W12-NEW).** `POST
   /api/spectator/handoff` resolves the caller's session
   cookie + mints via `JwtIssuingService`.
   `SpectatorHandoffTokenValidator` wraps
   `JwtValidationService` with the per-game scope check.
   Algorithm follows operator config (HS256 or RS256);
   kid-lookup + multi-key fallback applies.
19. **Single EF migration
   `Phase_K_W12_Replays_Brackets_SignalRSeq` across 3
   providers Sqlite/Postgres/SqlServer (W12-NEW)** adds all
   three W12 EF tables (`Replays`, `BracketRecords`,
   `SignalRSequenceEntries`) in one migration. Convention:
   when a wave ships multiple EF entities, bundle into a
   single named migration per provider — reduces
   migration-history churn + simplifies forward/backward
   rollouts.
20. **`NamespaceTransformer + unsetOnly: true` is the
   canonical cross-namespace kustomize pattern (W12-NEW).**
   Documented `docs/prod-cutover.md §4`. Rejected
   alternatives: JSON6902 patch, strategic-merge patch,
   Kustomize Component, sub-base inclusion, `replacements:`
   directive, two-overlay split — all rejected for ordering
   / propagation / ergonomic reasons.
21. **`optional: true` on every cutover-window envFrom mount
   for ESO-materialised Secrets (W12-NEW).** Preserves
   cutover-safe fall-through if ESO hasn't yet hydrated the
   Secret. Layered defaults — cutover is a sequence of
   flips, not a big-bang. `optional: false` flip
   post-cutover (W13+) once cluster reaches steady-state.
22. **Additive infra surface gated by EMPTY default
   (W12-NEW).** When a deliverable depends on out-of-lane
   work (e.g. another agent's cluster lifecycle), ship with
   an EMPTY default so `terraform plan` shows ZERO diff vs
   the prior wave's baseline. W7 → W12 multi-region
   transition is the reference; future multi-region work
   (RDS replication, S3 replication) should mirror.
23. **OUT-OF-BAND → IN-BAND file-header flip is mandatory
   (W12-NEW).** When a hand-off ships with an "OUT-OF-BAND"
   qualifier in its file header, the next wave wiring it
   in-band MUST update the header. Stale headers are worse
   than no headers — operators trust file headers as a quick
   sanity check.
24. **Multiple narrow NetworkPolicies over one wide policy
   (W12-NEW).** Reviewers audit each policy's allow-list
   independently. Chart upgrades that add new workloads
   become explicit. W12 Argo Rollouts split into 3
   (dashboard-ingress, controller-egress, dashboard-egress)
   because controller + dashboard have distinct egress
   profiles.
25. **Rehearsal harness needs TWO documented runs before
   scheduled-cadence promotion (W12-NEW).** First run
   validates the harness; second run validates that the
   harness is REPEATABLE. W12 JWT rehearsal #2 at 3:48 (39 %
   faster than W11 #1 at 6:12 thanks to Bishop's JWKS-cache
   pre-warm) confirms GA-ready — promote to `schedule: cron`
   in W13+.
26. **Cutover docs follow the 5-section
   `docs/<cutover>-cutover.md` template (W12-NEW).** Each
   cutover gets its own forward-pointing doc rather than
   appending sections to existing runbooks. Existing
   runbooks back-reference into the cutover doc.
27. **Single-wave EF-bundle migration convention
   (W12-NEW).** Multiple EF entities shipping in the same
   wave bundle into one named migration per provider (e.g.
   `Phase_K_W12_Replays_Brackets_SignalRSeq`). Reduces
   migration-history churn + simplifies forward/backward
   rollouts.
28. **DbSerial migration follow-through pattern (W12-NEW).**
   25-row candidate inventory + 3-parallel flake-detection
   methodology + Reads/Writes split proposal at
   `Phase_K_W12/Vasquez/db-serial-candidates.md` +
   `docs/test-architecture.md §3.1.1` + `§3.1.2`. Bishop
   tags candidates from W13+; flake harness wires into
   `.github/workflows/backend-tests.yml`; Reads/Writes split
   unlocks ~40 % of suite for parallel exec.
29. **Visual-regression methodology at 2 % pixel diff
   (W12-NEW).** `toHaveScreenshot({ maxDiffPixelRatio: 0.02
   })` with the pre-flight checklist (viewport pin,
   animations frozen, `document.fonts.ready` await).
   `docs/test-architecture.md §5` NEW. Reference spec
   `manifest-screenshots-visual.spec.ts`.
30. **Wave1ThroughKW12 supersedes Wave1ThroughKW11** as the
   regression sweep target. `git mv` preserves history per
   the W9 + W11 rename precedent. W11 self-lane tests
   softened to accept EITHER class name (forward-stage
   tolerant).
31. **Coordinator-direct interventions: ZERO for 7
   consecutive waves (W6-W12).** Pattern is now
   production-grade — agents own their rollup commits
   end-to-end; Scribe sweeps observe + archive only.
...
5. **Load-test reminder workflow handoff (Hudson W13+
   lead).** Close the cadence-automation gap; the W12
   manifest is operator-triggered, the cadence rules are
   narrative.
