...
📌 Team update (2026-05-13T10-00-00Z): Phase 3 wave complete. Vitest infra landed: 47 frontend tests GREEN across reducer, bridge, signalrClient, mock hook (jsdom + @testing-library). Vasquez locked v1.2 spec (winner-becomes-dealer); Bishop fixed 5 backend bugs + 24 tests (203 passing); Hicks shipped lobby + claim UX + fixes (48 tests). All merged to main in PR #25 (SHA a03feda). Next wave: refactor useLiveChangshaGame for testability, component tests, 16-hand E2E. See `.squad/orchestration-log/2026-05-13T10-00-hudson.md`.


### ─── Summarized from history.md on 2026-08-06 (≥15360 HARD GATE) ───
> 2026-04-20 (Changsha Test Catalog) and 2026-05-08 (P0-coverage) were already archived above (identical/fuller copies); only the 2026-05-13 Phase 5a entry + its two 2026-07-27 team updates were newly relocated this turn.

### 2026-05-13: Phase 5a — Frontend Test Coverage
- **`index.html` static-fixture parse pattern for jsdom-unreachable DOM contracts.** The canonical sidebar-hide implementation lives in an inline `<script>` in `index.html` — unreachable from vitest's jsdom. Static-fixture regex assertions cover the source-level invariant (`#sidebar { display:none }`, `.seat-buttons { display:none }`, `has('embedded')`, `setAttribute('data-changsha-embedded','1')`). Runtime behavior is documented as manual-repro (visit `http://localhost:5114/autotable/?embedded=1`, verify sidebar is hidden; visit `http://localhost:5114/autotable/`, verify it's visible). Static checks pin the contract; manual covers observability — a practical boundary for test investment.
- **Helper-export pinning provides value-stability coverage when full React render is overkill.** `buildAutotableIframeSrc(gameId, seatIndex?)` is a simple URL builder exported from `ChangshaTablePage.tsx`. Rather than rendering the component (would require mocking Fluent UI providers + `useChangshaGame` state), the test directly imports and tests the helper with value-stability assertions (`same inputs → identical strings`, `distinct inputs → distinct outputs`). This covers the contract Hicks wires into `useMemo([state.gameId, userSeat])` without the render-infra overhead. A component-render identity test (via `@testing-library/react`) is noted as optional Phase 5b hardening.

📌 Team update (2026-07-27T01-56-23-811-07-00): Your #128 (WP-F/#122 real-UI playability gate) hit Ripley's FINAL independent gate -> REJECT on main `ab3687d` (real-UI e2e + playability-gate FAIL); you are locked out, Bishop is revision owner. #128 was squash-merged anyway during a GitHub PR-sync glitch -> main RED. Your gate found P0 #132 & #134 (both fixed/approved) and NEW P0 #139 (the #135 claim fix hangs human-vs-bots at hand 1); your unpushed claim-exercise gate `76acb68` detects it. — recorded by Scribe (decisions.md §2026-07-27).

📌 Team update (2026-07-27T01-56-23-811-07-00, late-arrival addendum): **#137/#139 diagnosis: (b) PRODUCT DEFECT** (not a gate gap) -> owner **Bishop**. Ralph's 'human Hu win -> gate gap' premise DISPROVEN (human PASSED all 6 claims; Hu never offered). #137 is caused by #139: #135/`8aaa37b` (WS claim parser) wedges the runtime/bot-scheduler even when the human only passes. Claim-exercise gate `76acb68` (unpushed) detects it. — recorded by Scribe.
